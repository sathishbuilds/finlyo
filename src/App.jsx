...
function scoreLabel(s){
  ...
  return{label:"Start Here",color:"#C0312B",msg:"Add your income to unlock your full picture."};
}
...
function genInsights(income,curExp,prevExp,loans,savPct){
  ...
  if(prevTotal>0&&curTotal>0){const diff=curTotal-prevTotal;const pct=Math.abs((diff/prevTotal)*100).toFixed(0);if(diff<0)ins.push({type:"positive",icon:"↓",text:"Spent "+pct+"% less than last month. Nice discipline."});else if(diff>0)ins.push({type:"warning",icon:"↑",text:"Spending is up "+pct+"% from last month."});else ins.push({type:"neutral",icon:"→",text:"Spending is consistent with last month."});}
  if(income>0){const sp=(curTotal/income)*100;if(sp<=50)ins.push({type:"positive",icon:"✓",text:"On track this month. Spending is healthy."});else if(sp<=70)ins.push({type:"warning",icon:"!",text:"Spending is getting high. Watch discretionary costs."});else ins.push({type:"negative",icon:"↑",text:"Overspending this month. Review your budget."});}
  ...
  if(income>0){if(sav>=30)ins.push({type:"positive",icon:"★",text:"Saving "+sav+"% of income. Above the 30% target."});else if(sav>=20)ins.push({type:"positive",icon:"★",text:"Saving "+sav+"% of income. Try pushing to 30%."});else if(sav>=0)ins.push({type:"warning",icon:"!",text:"Saving only "+sav+"% of income. Target is 20%."});else ins.push({type:"negative",icon:"↓",text:"Spending more than you earn this month."});}
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Plus Jakarta Sans','Inter',sans-serif",color:C.ink,overflowX:"hidden",maxWidth:"100vw"}}>
      ...
      <nav className="tabbar" style={{background:C.white,borderBottom:"1px solid "+C.border,padding:"0 10px",display:"flex",overflowX:"auto",scrollbarWidth:"none"}}>
        ...
        <div style={{position:"relative",marginLeft:"auto",flexShrink:0}}>
          <button onClick={()=>setShowMore(!showMore)} style={{padding:"10px 14px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:"12px",fontWeight:SECONDARY_TABS.includes(tab)?"700":"400",color:SECONDARY_TABS.includes(tab)?C.blue:C.inkLo,borderBottom:SECONDARY_TABS.includes(tab)?"2px solid "+C.blue:"2px solid transparent",whiteSpace:"nowrap"}}>
            {SECONDARY_TABS.includes(tab)?tab:"More"} &#9660;
          </button>
          {showMore&&(
            <div style={{position:"absolute",right:0,top:"43px",background:C.white,border:"1px solid "+C.border,borderRadius:"12px",overflow:"hidden",zIndex:200,minWidth:"140px",boxShadow:"0 8px 32px rgba(0,0,0,0.14)"}}>
              {SECONDARY_TABS.map(t=>(
                <button key={t} onClick={()=>switchTab(t)} style={{display:"block",width:"100%",padding:"11px 16px",border:"none",background:tab===t?C.blueSoft:"transparent",color:tab===t?C.blue:C.ink,cursor:"pointer",fontFamily:"inherit",fontSize:"13px",textAlign:"left",borderBottom:"1px solid "+C.border,fontWeight:tab===t?"700":"400"}}>{t}</button>
              ))}
            </div>
          )}
        </div>
