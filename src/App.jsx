...
function Finlyo({user,darkMode,onOpenSettings}){
  ...
  const[csvData,setCsvData]=useState([]);   // parsed CSV rows pending review
  ...
  // ─── CSV IMPORT ──────────────────────────────────────────────────────────
  // Supported formats: HDFC, SBI, ICICI, Axis, Kotak, generic debit/credit CSV
  // Columns detected: Date, Description/Narration, Debit/Withdrawal, Credit/Deposit
  const parseCSV=(text)=>{
    const lines=text.split('\n').map(l=>l.trim()).filter(Boolean);
    if(lines.length<2)return{error:'File appears empty or unreadable.'};
    // Detect separator
    const sep=lines[0].includes('\t')?'\t':',';
    const headers=lines[0].split(sep).map(h=>h.replace(/["\']/g,'').trim().toLowerCase());
    // Column index detection
    const fi=(candidates)=>candidates.reduce((found,c)=>{if(found>=0)return found;const i=headers.findIndex(h=>h.includes(c));return i;},-1);
    const dateIdx=fi(['date','txn date','transaction date','value date','posting date']);
    const descIdx=fi(['description','narration','particulars','remarks','details','transaction remarks','tran particular']);
    const debitIdx=fi(['debit','withdrawal','dr','debit amount','withdraw','amount(dr)','amount (dr)']);
    const creditIdx=fi(['credit','deposit','cr','credit amount','amount(cr)','amount (cr)']);
    const amtIdx=fi(['amount','transaction amount']);
    if(dateIdx<0)return{error:'Could not find a Date column. Please check your CSV format.'};
    if(descIdx<0)return{error:'Could not find a Description/Narration column.'};
    if(debitIdx<0&&amtIdx<0)return{error:'Could not find a Debit/Amount column.'};
    const rows=[];
    for(let i=1;i<lines.length;i++){
      const cols=lines[i].split(sep).map(c=>c.replace(/["\']/g,'').trim());
      if(cols.length<2)continue;
      const rawAmt=debitIdx>=0?cols[debitIdx]:cols[amtIdx];
      const amt=parseFloat((rawAmt||'').replace(/[,\s]/g,''));
      if(!amt||amt<=0)continue; // skip credits & zero rows
      const rawDate=cols[dateIdx]||'';
      const desc=(cols[descIdx]||'').substring(0,60);
      // Auto-categorise based on keywords
      const d=desc.toLowerCase();
      let catId='misc',catLabel='Miscellaneous';
      if(/zomato|swiggy|dineout|restaurant|hotel|cafe|food|biryani|pizza|burger/.test(d)){catId='dining';catLabel='Dining';}
      else if(/amazon|flipkart|myntra|ajio|nykaa|meesho|shopping|mart|store/.test(d)){catId='shopping';catLabel='Shopping';}
      else if(/uber|ola|rapido|metro|bus|train|irctc|petrol|fuel|auto|cab/.test(d)){catId='transport';catLabel='Transport';}
      else if(/electricity|water|gas|jio|airtel|bsnl|broadband|wifi|bill|utility/.test(d)){catId='utilities';catLabel='Utilities';}
      else if(/rent|landlord|housing/.test(d)){catId='rent';catLabel='Rent / Mortgage';}
      else if(/hospital|doctor|pharmacy|medical|health|apollo|fortis|clinic/.test(d)){catId='health';catLabel='Healthcare';}
      else if(/school|college|university|fee|tuition|course|udemy|coursera/.test(d)){catId='education';catLabel='Education';}
      else if(/netflix|hotstar|prime|spotify|youtube|disney|movie|cinema|pvr/.test(d)){catId='entertain';catLabel='Entertainment';}
      else if(/emi|loan|hdfc|icici|sbi|axis|kotak|bajaj|finance/.test(d)){catId='emi';catLabel='Loan EMI';}
      else if(/insurance|lic|policy|premium/.test(d)){catId='insurance';catLabel='Insurance';}
      else if(/grocery|bigbasket|grofers|blinkit|zepto|instamart|dmart|supermarket/.test(d)){catId='grocery';catLabel='Groceries';}
      else if(/mutual fund|sip|mf|growth fund|invest/.test(d)){catId='savings';catLabel='Savings';}
      rows.push({id:Date.now()+i,catId,catLabel,amount:amt,note:desc,rawDate,selected:true});
    }
    if(rows.length===0)return{error:'No debit transactions found in this file.'};
    return{rows};
  };
  ...
  const handleCSVFile=(e)=>{
    ...
    reader.onload=(ev)=>{
      ...
      if(result.error){setCsvError(result.error);setCsvData([]);}
      else{setCsvData(result.rows);setCsvError('');}
    };
    reader.readAsText(file);
  };

  const importSelectedCSV=async()=>{
    const toImport=csvData.filter(r=>r.selected);
    if(toImport.length===0)return;
    const existing=md.expenses||[];
    const newExps=toImport.map(r=>({id:r.id,catId:r.catId,catLabel:r.catLabel,amount:r.amount,note:r.note}));
  ...
  const css="@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=DM+Mono:wght@300;400;500&display=swap');"
    ...
    +".num,.mono-num{font-family:'Plus Jakarta Sans',sans-serif;font-variant-numeric:tabular-nums;font-feature-settings:'tnum' 1;}"
      ...
      <main style={{padding:"14px 13px 56px",maxWidth:"640px",margin:"0 auto"}} onClick={()=>{showCur&&setShowCur(false);showMore&&setShowMore(false);}}>
        ...
        {tab==="Budget"&&<div className="fade">
          ...
          <div className="hc" style={{background:darkMode?"linear-gradient(135deg,#1A3A6A,#2A5090)":"linear-gradient(135deg,#2B6CB0,#3A7FCC)",borderRadius:"16px",padding:"18px 16px",marginBottom:"10px",color:"#fff",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{marginBottom:"12px"}}><div style={{fontSize:"28px",fontWeight:"800",letterSpacing:"-1px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmt(bal,S,cur.code)}</div>{Math.abs(bal)>=10000&&<div style={{fontSize:"10px",opacity:.65,fontFamily:"'DM Mono',monospace",marginTop:"2px"}}>~{fmtCompact(bal,S,cur.code)}</div>}</div>
          ...
          <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"13px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"11px"}}>
              ...
              <div style={{display:"flex",gap:"6px"}}><button onClick={()=>{setCsvData([]);setCsvError('');setModal("csv");}} style={{padding:"5px 10px",borderRadius:"20px",border:"1px solid "+C.borderMid,background:C.bg,color:C.inkMid,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"10px",letterSpacing:"1px"}}>↑ CSV</button><button onClick={()=>setModal("expense")} style={{padding:"5px 12px",borderRadius:"20px",border:"1px solid "+C.blue,background:C.blueSoft,color:C.blue,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"10px",letterSpacing:"1px",fontWeight:"600"}}>+ ADD</button></div>
            </div>
        ...
        {tab==="Score"&&<div className="fade">
          ...
          {md.income>0&&<div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"14px 16px",marginBottom:"10px"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:"6px",fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace"}}><span>Spent: {fmt(totExp,S,cur.code)}</span><span>Saved: {fmt(totalSaved,S,cur.code)}</span></div>
          </div>}
...
{modal==="csv"&&<><div style={{fontSize:"15px",fontWeight:"700",color:C.ink,marginBottom:"4px"}}>Import from Bank CSV</div><div style={{fontSize:"11px",color:C.inkLo,marginBottom:"14px",lineHeight:"1.5"}}>Download your bank statement as CSV (from HDFC NetBanking, SBI, ICICI, Axis etc.) then upload it here. Transactions are auto-categorised — review and deselect any you don't want.</div><div style={{background:C.blueSoft,border:"2px dashed "+C.blue+"44",borderRadius:"12px",padding:"18px",textAlign:"center",marginBottom:"12px",cursor:"pointer"}} onClick={()=>document.getElementById('csvFileInput').click()}><div style={{fontSize:"22px",marginBottom:"6px"}}>📄</div><div style={{fontSize:"13px",fontWeight:"600",color:C.blue,marginBottom:"3px"}}>Tap to select CSV file</div><div style={{fontSize:"11px",color:C.inkLo}}>Works with HDFC, SBI, ICICI, Axis, Kotak and most Indian banks</div><input id="csvFileInput" type="file" accept=".csv,.txt" onChange={handleCSVFile} style={{display:"none"}}/></div>{csvError&&<div style={{background:C.redSoft,border:"1px solid "+C.red+"33",borderRadius:"10px",padding:"10px 13px",fontSize:"12px",color:C.red,marginBottom:"12px"}}>{csvError}</div>}{csvData.length>0&&<><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}><div style={{fontSize:"12px",fontWeight:"600",color:C.ink}}>{csvData.filter(r=>r.selected).length} of {csvData.length} selected</div><div style={{display:"flex",gap:"6px"}}><button onClick={()=>setCsvData(d=>d.map(r=>({...r,selected:true})))} style={{fontSize:"10px",color:C.blue,background:"none",border:"none",cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>ALL</button><button onClick={()=>setCsvData(d=>d.map(r=>({...r,selected:false})))} style={{fontSize:"10px",color:C.inkLo,background:"none",border:"none",cursor:"pointer",fontFamily:"'DM Mono',monospace"}}>NONE</button></div></div><div style={{maxHeight:"260px",overflowY:"auto",marginBottom:"14px",border:"1px solid "+C.border,borderRadius:"10px"}}>{csvData.map((row,i)=>(<div key={row.id} onClick={()=>setCsvData(d=>d.map((r,j)=>j===i?{...r,selected:!r.selected}:r))} style={{display:"flex",alignItems:"center",gap:"10px",padding:"9px 11px",borderBottom:i<csvData.length-1?"1px solid "+C.border:"none",background:row.selected?C.blueSoft:"transparent",cursor:"pointer"}}><div style={{width:"16px",height:"16px",borderRadius:"4px",border:"1.5px solid "+(row.selected?C.blue:C.borderMid),background:row.selected?C.blue:"transparent",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>{row.selected&&<span style={{color:"#fff",fontSize:"10px",lineHeight:1}}>✓</span>}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:"12px",color:C.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{row.note}</div><div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace"}}>{row.catLabel} · {row.rawDate}</div></div><div style={{fontSize:"12px",fontWeight:"700",color:C.red,fontFamily:"'Plus Jakarta Sans',sans-serif",flexShrink:0}}>-{fmt(row.amount,S,cur.code)}</div></div>))}</div><button onClick={importSelectedCSV} style={{width:"100%",padding:"12px",borderRadius:"10px",border:"none",background:C.blue,color:"#fff",cursor:"pointer",fontFamily:"'DM Mono',monospace",fontWeight:"600",fontSize:"12px",letterSpacing:"2px"}}>IMPORT {csvData.filter(r=>r.selected).length} TRANSACTIONS</button></>}</>}
