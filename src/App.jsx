...
const LIGHT={bg:"#F7F8FC",white:"#FFFFFF",card:"#FFFFFF",input:"#F0F2F8",border:"#E4E7F2",borderMid:"#CDD2E8",ink:"#0D1117",inkMid:"#3A4560",inkLo:"#8892AA",blue:"#4F46E5",blueSoft:"#EEF2FF",blueMid:"#6366F1",green:"#059669",greenSoft:"#ECFDF5",red:"#DC2626",redSoft:"#FEF2F2",amber:"#D97706",amberSoft:"#FFFBEB",steel:"#64748B",surface:"#FFFFFF",surfaceHover:"#F7F8FC"};
const DARK={bg:"#09090F",white:"#111118",card:"#111118",input:"#0D0D14",border:"#1E1E2E",borderMid:"#2A2A3E",ink:"#F2F4FF",inkMid:"#8B93B8",inkLo:"#3F4565",blue:"#6366F1",blueSoft:"#1A1A3E",blueMid:"#818CF8",green:"#10B981",greenSoft:"#052E1C",red:"#F87171",redSoft:"#2D0F0F",amber:"#FBBF24",amberSoft:"#1C1200",steel:"#64748B",surface:"#161622",surfaceHover:"#1A1A28"};
...
function calcHealthScore(income,expenses,loans,goals,invests){
  ...
  const totExp=expenses.reduce((s,e)=>e.catId!=="savings"?s+e.amount:s,0);
  const intentionalSav=expenses.reduce((s,e)=>e.catId==="savings"?s+e.amount:s,0);
  const totalSaved=intentionalSav+Math.max(0,income-totExp-intentionalSav);
  const savPct=(totalSaved/income)*100;
...
function genStory(income,expenses,prevExpenses,loans,goals,invests,savPct,budgets,mo,yr,S,curCode){
  ...
  const totExp=expenses.reduce((s,e)=>e.catId!=="savings"?s+e.amount:s,0);
  const prevTot=prevExpenses.reduce((s,e)=>e.catId!=="savings"?s+e.amount:s,0);
  const sav=parseFloat(savPct);
  const totInv=invests.reduce((s,i)=>s+i.invested,0);
  const totCur=invests.reduce((s,i)=>s+i.current_value,0);
  const pnl=totCur-totInv;
          ...
          {cats.map(c=>(
            <div key={c.label} style={{marginBottom:"9px"}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:"3px"}}>
                ...
                <span style={{fontSize:"10px",color:c.color,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums",fontWeight:"600"}}>{c.val}/{c.max}</span>
              </div>
...
function Auth({darkMode,onToggleTheme,onLogin}){
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",fontFamily:"'Inter','Plus Jakarta Sans',sans-serif",color:C.ink}}>
...
function Settings({user,darkMode,onToggleTheme,onBack,onSignOut}){
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter','Plus Jakarta Sans',sans-serif",color:C.ink}}>
...
function Finlyo({user,darkMode,onOpenSettings}){
  ...
  const[spendView,setSpendView]=useState("overview"); // "overview" | "breakdown" | "patterns"
  const[quickAmt,setQuickAmt]=useState("");
  const[quickCustomLabel,setQuickCustomLabel]=useState("");
  const[recurringExp,setRecurringExp]=useState(()=>{try{return JSON.parse(localStorage.getItem("fk_recurring")||"[]");}catch{return[];}});
  ...
  const totExp=(md.expenses||[]).reduce((s,e)=>e.catId!=="savings"?s+e.amount:s,0);
  const intentionalSavings=(md.expenses||[]).reduce((s,e)=>e.catId==="savings"?s+e.amount:s,0);
  const bal=(md.income||0)-totExp-intentionalSavings;
  const totalSaved=intentionalSavings+Math.max(0,bal);
  const savPct=md.income>0?((totalSaved/md.income)*100).toFixed(1):"0.0";
  const totInv=invests.reduce((s,i)=>s+i.invested,0);
  const totCur=invests.reduce((s,i)=>s+i.current_value,0);
  ...
  // Spending patterns by day of week — uses expense id as timestamp
  const spendingByDay=(()=>{
    const days=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];
    const totals=[0,0,0,0,0,0,0];
    const counts=[0,0,0,0,0,0,0];
    // Scan ALL budget months for patterns
    Object.values(budgets).forEach(b=>{
      (b.expenses||[]).forEach(e=>{
        // id is Date.now() at time of entry — extract day of week
        const ts=typeof e.id==="number"?e.id:parseInt(e.id);
        if(ts>1000000000000){ // valid ms timestamp (after 2001)
    ...
    const highestDay=totals.indexOf(Math.max(...totals));
    const lowestActiveDay=totals.reduce((min,v,i)=>v>0&&v<totals[min]?i:min,highestDay);
    return{days,totals,counts,maxAmt,highestDay,lowestActiveDay};
  })();

  // Expense category breakdown for spending pie
  const expCatBreakdown=(()=>{
    const COLORS=["#6366F1","#F59E0B","#10B981","#F87171","#34D399","#FB923C","#60A5FA","#A78BFA","#2DD4BF","#F472B6","#FBBF24","#94A3B8"];
    const items=Object.entries(catTotals)
      .map(([catId,amount])=>{
        const cat=EXPENSE_CATS.find(c=>c.id===catId)||{label:catId};
        return{catId,label:cat.label,amount};
      })
      .filter(x=>x.amount>0)
      .sort((a,b)=>b.amount-a.amount);
    const total=items.reduce((s,x)=>s+x.amount,0);
    return items.map((x,i)=>({...x,pct:total>0?((x.amount/total)*100):0,color:COLORS[i%COLORS.length]}));
  })();
  ...
  const savingStreak=(()=>{
    ...
    for(let i=0;i<12;i++){
      ...
      const b=budgets[k];
      if(!b||!b.income)break;
      const exp=(b.expenses||[]).reduce((s,e)=>e.catId!=="savings"?s+e.amount:s,0);
      const intSav=(b.expenses||[]).reduce((s,e)=>e.catId==="savings"?s+e.amount:s,0);
      const saved=intSav+Math.max(0,b.income-exp-intSav);
      if((saved/b.income)*100>=20)streak++;
      else break;
    }
    return streak;
  })();
  ...
  // Net worth
  const netWorth=totCur+(goals.reduce((s,g)=>s+g.saved,0))-loans.reduce((s,l)=>s+l.remaining,0);

  // Subscription total (expenses with catId=insurance or entertain tagged recurring)
  const monthlySubscriptions=recurringExp.reduce((s,r)=>s+r.amount,0);
  ...
  // Auto-apply recurring expenses on first load of current month
  useEffect(()=>{
    if(!uid||!recurringExp.length||loading)return;
    const alreadyApplied=localStorage.getItem("fk_rec_"+mk);
    if(alreadyApplied)return;
    const cur=budgets[mk];
    if(!cur?.id)return;
    const existing=cur.expenses||[];
    const toAdd=recurringExp.filter(r=>{
      return !existing.some(e=>e.catId===r.catId&&e.amount===r.amount&&e.note==="[recurring]");
    });
    if(toAdd.length===0){localStorage.setItem("fk_rec_"+mk,"1");return;}
    const newExps=toAdd.map(r=>({id:Date.now()+Math.random(),catId:r.catId,catLabel:r.catLabel,amount:r.amount,note:"[recurring]"}));
    const expenses=[...existing,...newExps];
    supabase.from("budgets").update({expenses}).eq("id",cur.id).then(()=>{
      setBudgets(p=>({...p,[mk]:{...p[mk],expenses}}));
      localStorage.setItem("fk_rec_"+mk,"1");
      showToast(newExps.length+" recurring expenses added.");
    });
  },[uid,mk,loading,budgets]);

  const saveRecurring=(cat,amount)=>{
    const entry={catId:cat.id,catLabel:cat.label,amount};
    const updated=[...recurringExp.filter(r=>r.catId!==cat.id),entry];
    setRecurringExp(updated);
    localStorage.setItem("fk_recurring",JSON.stringify(updated));
    showToast("Set as recurring every month.");
  };
  const removeRecurring=(catId)=>{
    const updated=recurringExp.filter(r=>r.catId!==catId);
    setRecurringExp(updated);
    localStorage.setItem("fk_recurring",JSON.stringify(updated));
  };
  ...
  const quickAdd=async()=>{
    ...
    if(!amt||amt<=0)return;
    if(quickCat==="custom"&&!quickCustomLabel.trim())return;
    const cat=EXPENSE_CATS.find(c=>c.id===quickCat)||{id:"misc",label:"Miscellaneous"};
    const resolvedLabel=quickCat==="custom"?(quickCustomLabel.trim()):cat.label;
    const resolvedId=quickCat==="custom"?("custom_"+Date.now()):cat.id;
    const newExp={id:Date.now(),catId:resolvedId,catLabel:resolvedLabel,amount:amt,note:resolvedLabel};
    const expenses=[...(md.expenses||[]),newExp];
    const ex=budgets[mk];
    if(ex?.id){await supabase.from("budgets").update({expenses}).eq("id",ex.id);}
    else{const{data}=await supabase.from("budgets").insert({user_id:uid,month_key:mk,income:0,expenses}).select().single();setBudgets(p=>({...p,[mk]:{id:data.id,income:0,expenses}}));}
    setBudgets(p=>({...p,[mk]:{...p[mk],expenses}}));
    setQuickAmt("");setQuickCustomLabel("");setQuickCat("dining");setShowQuick(false);
    showToast(resolvedLabel+" · "+S+amt+" logged.");
  };
  ...
  const css="@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&family=DM+Mono:wght@400;500&display=swap');"
    +"*{box-sizing:border-box;margin:0;padding:0;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;}"
    +"body{background:"+C.bg+";font-family:'Inter','Plus Jakarta Sans',sans-serif;}"
    ...
    +"@keyframes ring{0%{transform:scale(1);}20%{transform:scale(1.12);}40%{transform:scale(0.95);}60%{transform:scale(1.05);}80%{transform:scale(0.98);}100%{transform:scale(1);}}"
    // Card & interaction classes
    +".fade{animation:fadeUp 0.3s cubic-bezier(0.16,1,0.3,1) forwards;}"
    +".scaleIn{animation:scaleIn 0.25s cubic-bezier(0.16,1,0.3,1) forwards;}"
    +".card{background:"+C.card+";border:1px solid "+C.border+";border-radius:16px;transition:box-shadow 0.2s,transform 0.15s;}"
    +".card:hover{box-shadow:0 4px 24px rgba(0,0,0,0.07);transform:translateY(-1px);}"
    +".hc{transition:box-shadow 0.2s,transform 0.15s;}"
    +".hc:hover{box-shadow:0 4px 24px rgba(0,0,0,0.08);transform:translateY(-1px);}"
    +".hr:hover{background:"+C.input+"!important;}"
    +"button:active{transform:scale(0.97);}"
    +"input[type=range]{accent-color:"+C.blue+";width:100%;height:4px;}"
    ...
    // Pill chip style
    +".chip{display:inline-flex;align-items:center;padding:3px 10px;border-radius:99px;font-size:11px;font-weight:500;letter-spacing:0;}"
    // Number inputs — clean
  ...
  if(loading)return<div style={{background:C.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:"16px"}}><div style={{width:"36px",height:"36px",borderRadius:"10px",background:"linear-gradient(135deg,#4F46E5,#7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(79,70,229,0.4)"}}><svg width="18" height="18" viewBox="0 0 14 14" fill="none"><path d="M7 1L12 4V7C12 9.8 9.8 12.4 7 13C4.2 12.4 2 9.8 2 7V4L7 1Z" fill="white" fillOpacity="0.9"/></svg></div><div style={{fontSize:"13px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"3px"}}>LOADING</div></div>;
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter','Plus Jakarta Sans',sans-serif",color:C.ink,overflowX:"hidden",maxWidth:"100vw"}}>
      <style>{css}</style>
      ...
      {toast&&<div style={{position:"fixed",top:"16px",left:"50%",transform:"translateX(-50%)",zIndex:1000,background:darkMode?"rgba(30,30,46,0.95)":"rgba(13,17,23,0.92)",backdropFilter:"blur(16px)",color:"#fff",borderRadius:"99px",padding:"10px 20px",fontSize:"13px",fontWeight:"500",whiteSpace:"nowrap",maxWidth:"90vw",textAlign:"center",boxShadow:"0 8px 32px rgba(0,0,0,0.3)",animation:"slideDown .25s cubic-bezier(0.16,1,0.3,1)",border:"1px solid rgba(255,255,255,0.08)",letterSpacing:"-0.01em"}}>{toast}</div>}

      {/* QUICK ADD OVERLAY */}
      {showQuick&&<div onClick={()=>setShowQuick(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(8px)",zIndex:800,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
        <div onClick={e=>e.stopPropagation()} style={{background:C.white,borderRadius:"24px 24px 0 0",padding:"20px 18px 40px",width:"100%",maxWidth:"480px",boxShadow:"0 -8px 48px rgba(0,0,0,0.15)",animation:"slideDown 0.25s cubic-bezier(0.16,1,0.3,1)"}}>
          <div style={{width:"32px",height:"3px",background:C.borderMid,borderRadius:"99px",margin:"0 auto 18px"}}/>
          <div style={{fontSize:"13px",fontWeight:"600",color:C.inkLo,marginBottom:"12px",letterSpacing:"-0.01em"}}>Quick add expense</div>
          <div style={{background:C.input,borderRadius:"14px",padding:"16px",marginBottom:"12px",display:"flex",alignItems:"center",gap:"8px"}}>
            <span style={{fontSize:"22px",color:C.inkLo,fontFamily:"inherit",fontWeight:"300"}}>{S}</span>
            <input autoFocus type="number" value={quickAmt} onChange={e=>setQuickAmt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&quickAdd()} placeholder="0" style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:"32px",fontWeight:"700",color:C.ink,fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums"}}/>
          </div>
          <div style={{display:"flex",gap:"6px",flexWrap:"wrap",marginBottom:"12px"}}>
            {[{id:"dining",label:"🍔 Food"},{id:"transport",label:"🚗 Travel"},{id:"shopping",label:"🛍 Shopping"},{id:"grocery",label:"🛒 Grocery"},{id:"entertain",label:"🎬 Fun"},{id:"utilities",label:"⚡ Bills"},{id:"health",label:"💊 Health"},{id:"custom",label:"✏️ Custom"}].map(c=>(
              <button key={c.id} onClick={()=>setQuickCat(c.id)} style={{padding:"7px 12px",borderRadius:"99px",border:"1px solid "+(quickCat===c.id?C.blue:C.border),background:quickCat===c.id?C.blue:"transparent",color:quickCat===c.id?"#fff":C.inkMid,cursor:"pointer",fontSize:"12px",fontWeight:"500",transition:"all 0.15s"}}>{c.label}</button>
            ))}
          </div>
          {quickCat==="custom"&&<div style={{marginBottom:"12px"}}>
            <input autoFocus={false} type="text" value={quickCustomLabel} onChange={e=>setQuickCustomLabel(e.target.value)} placeholder="e.g. Juice, Bus ticket, Haircut..." style={{...inp,fontSize:"14px",borderRadius:"12px"}}/>
          </div>}
          <button onClick={quickAdd} style={{width:"100%",padding:"14px",borderRadius:"12px",border:"none",background:C.blue,color:"#fff",cursor:"pointer",fontSize:"15px",fontWeight:"600",letterSpacing:"-0.01em"}}>Log Expense</button>
        </div>
      </div>}

      {/* HEADER — Premium glass */}
      <header style={{background:darkMode?"rgba(9,9,15,0.92)":"rgba(255,255,255,0.92)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid "+C.border,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:90}}>
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          <div style={{width:"30px",height:"30px",borderRadius:"10px",background:"linear-gradient(135deg,#4F46E5,#7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 2px 8px rgba(79,70,229,0.35)"}}><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M7 1L12 4V7C12 9.8 9.8 12.4 7 13C4.2 12.4 2 9.8 2 7V4L7 1Z" fill="white" fillOpacity="0.9"/></svg></div>
          <div><div style={{fontSize:"15px",fontWeight:"700",color:C.ink,lineHeight:1,letterSpacing:"-0.4px"}}>Finlyo</div><div style={{fontSize:"9px",color:C.inkLo,letterSpacing:"0.12em",fontFamily:"'DM Mono',monospace",lineHeight:1.4,marginTop:"1px"}}>MONEY CLARITY</div></div>
        </div>
        <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
          <button onClick={onOpenSettings} style={{padding:"6px 12px",borderRadius:"99px",border:"1px solid "+C.border,background:"transparent",color:C.inkMid,cursor:"pointer",fontSize:"12px",fontWeight:"500",letterSpacing:"-0.01em"}}>Settings</button>
          <div style={{position:"relative"}} onClick={e=>e.stopPropagation()}>
            <button onClick={()=>setShowCur(!showCur)} style={{display:"flex",alignItems:"center",gap:"5px",padding:"6px 11px",borderRadius:"99px",border:"1px solid "+C.border,background:"transparent",color:C.inkMid,cursor:"pointer",fontSize:"12px",fontWeight:"500"}}><span style={{fontSize:"14px"}}>{cur.flag}</span><span style={{fontFamily:"'DM Mono',monospace",fontSize:"11px",letterSpacing:"0.05em"}}>{cur.code}</span><span style={{fontSize:"7px",color:C.inkLo}}>▾</span></button>
            {showCur&&<div onClick={e=>e.stopPropagation()} style={{position:"absolute",right:0,top:"40px",background:C.white,border:"1px solid "+C.border,borderRadius:"16px",overflow:"hidden",zIndex:300,width:"200px",maxHeight:"280px",overflowY:"auto",boxShadow:"0 16px 48px rgba(0,0,0,0.16)",animation:"scaleIn 0.15s ease"}}>{CURRENCIES.map(c=><button key={c.code} onClick={()=>{setCur(c);setShowCur(false);}} style={{display:"flex",alignItems:"center",gap:"10px",width:"100%",padding:"10px 14px",border:"none",background:cur.code===c.code?C.blueSoft:"transparent",color:C.ink,cursor:"pointer",fontFamily:"inherit",fontSize:"13px",textAlign:"left",borderBottom:"1px solid "+C.border,transition:"background 0.1s"}}><span style={{fontSize:"16px"}}>{c.flag}</span><span style={{flex:1,fontWeight:cur.code===c.code?"600":"400"}}>{c.country}</span><span style={{color:C.blue,fontFamily:"'DM Mono',monospace",fontSize:"11px",fontWeight:"500"}}>{c.symbol}</span></button>)}</div>}
          </div>
        </div>
      </header>

      {/* TAB BAR */}
      <nav className="tabbar" style={{background:darkMode?"rgba(9,9,15,0.92)":"rgba(255,255,255,0.92)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid "+C.border,padding:"0 12px",display:"flex",overflowX:"auto",scrollbarWidth:"none",gap:"2px"}}>
        {PRIMARY_TABS.map(t=>(
          <button key={t} onClick={()=>switchTab(t)} style={{flexShrink:0,padding:"11px 15px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",fontWeight:tab===t?"600":"400",color:tab===t?C.blue:C.inkLo,borderBottom:tab===t?"2px solid "+C.blue:"2px solid transparent",whiteSpace:"nowrap",transition:"color 0.15s, border-color 0.15s",letterSpacing:"-0.01em"}}>{t}</button>
        ))}
        <button onClick={()=>setShowMore(s=>!s)} style={{marginLeft:"auto",flexShrink:0,padding:"11px 15px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",fontWeight:["Tools","Rates","Goals"].includes(tab)?"600":"400",color:["Tools","Rates","Goals"].includes(tab)?C.blue:C.inkLo,borderBottom:["Tools","Rates","Goals"].includes(tab)?"2px solid "+C.blue:"2px solid transparent",whiteSpace:"nowrap",letterSpacing:"-0.01em",transition:"color 0.15s"}}>
          Tools ▾
        </button>
      </nav>
      {/* TOOLS DROPDOWN — rendered outside nav to avoid clipping */}
      {showMore&&<>
        <div onClick={()=>setShowMore(false)} style={{position:"fixed",inset:0,zIndex:490}}/>
        <div style={{position:"fixed",right:"12px",top:"100px",background:C.white,border:"1px solid "+C.border,borderRadius:"16px",overflow:"hidden",zIndex:500,minWidth:"180px",boxShadow:"0 16px 48px rgba(0,0,0,0.18)",animation:"scaleIn 0.15s ease"}}>
          {[{label:"Interest Rates",value:"Rates",icon:"📊"},{label:"EMI Calculator",value:"Tools",icon:"🧮"},{label:"Goals",value:"Goals",icon:"🎯"}].map((item,i,arr)=>(
            <button key={item.value} onClick={()=>{switchTab(item.value);setShowMore(false);}} style={{display:"flex",alignItems:"center",gap:"12px",width:"100%",padding:"14px 18px",border:"none",background:tab===item.value?C.blueSoft:"transparent",color:tab===item.value?C.blue:C.ink,cursor:"pointer",fontFamily:"inherit",fontSize:"14px",textAlign:"left",borderBottom:i<arr.length-1?"1px solid "+C.border:"none",fontWeight:tab===item.value?"600":"400"}}><span style={{fontSize:"16px"}}>{item.icon}</span><span>{item.label}</span></button>
          ))}
        </div>
      </>}

      {tab==="Budget"&&<button onClick={()=>{setShowQuick(true);setQuickAmt("");}} style={{position:"fixed",bottom:"24px",right:"20px",zIndex:700,width:"52px",height:"52px",borderRadius:"50%",background:"linear-gradient(135deg,"+C.blue+","+C.blueMid+")",color:"#fff",border:"none",cursor:"pointer",fontSize:"22px",fontWeight:"300",boxShadow:"0 4px 20px "+C.blue+"55",display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1}}>+</button>}

      <main style={{padding:"14px 13px 80px",maxWidth:"640px",margin:"0 auto"}} onClick={()=>{showCur&&setShowCur(false);showMore&&setShowMore(false);}}>

        {/* ═══ BUDGET ═══ */}
        {tab==="Budget"&&<div className="fade">
          {/* Net worth + streak row */}
          <div style={{display:"flex",gap:"8px",marginBottom:"14px"}}>
            <div style={{flex:1,background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"10px 13px"}}>
              <div style={{fontSize:"9px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1.5px",marginBottom:"3px"}}>NET WORTH</div>
              <div style={{fontSize:"16px",fontWeight:"700",color:netWorth>=0?C.green:C.red,fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums",letterSpacing:"-0.03em"}}>{fmt(netWorth,S,cur.code)}</div>
            </div>

          </div>
          <MonthNav/>
          {/* Daily pulse — if income set */}
          {md.income>0&&(()=>{
            const daysInMonth=new Date(yr,mo+1,0).getDate();
            const today=new Date();
            const dayOfMonth=(today.getMonth()===mo&&today.getFullYear()===yr)?today.getDate():daysInMonth;
            const dailyBudget=md.income/daysInMonth;
            const spentToday=totExp/daysInMonth; // approximation
            const pct=Math.min(100,(spentToday/dailyBudget)*100);
            const ok=pct<80;
            return<div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"11px 13px",marginBottom:"10px",display:"flex",alignItems:"center",gap:"10px"}}>
              <div style={{flex:1}}>
                <div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"4px"}}>DAILY BUDGET</div>
                <div style={{background:C.bg,borderRadius:"99px",height:"4px",overflow:"hidden"}}>
                  <div style={{height:"100%",width:pct+"%",borderRadius:"99px",background:ok?"linear-gradient(90deg,"+C.green+",#34D399)":"linear-gradient(90deg,"+C.amber+","+C.red+")",transition:"width 0.6s ease"}}/>
                </div>
              </div>
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:"13px",fontWeight:"700",color:ok?C.green:C.amber,fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmt(Math.max(0,dailyBudget-spentToday),S,cur.code)}</div>
                <div style={{fontSize:"9px",color:C.inkLo,fontFamily:"'DM Mono',monospace"}}>left / day</div>
              </div>
            </div>;
          })()}
          ...
          <div className="hc" style={{background:darkMode?"linear-gradient(135deg,#1a1a3e,#2d2b70,#1e3a5f)":"linear-gradient(135deg,#4F46E5,#6366F1,#4338CA)",borderRadius:"16px",padding:"18px 16px",marginBottom:"10px",color:"#fff",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{marginBottom:"12px"}}><div style={{fontSize:"28px",fontWeight:"800",letterSpacing:"-1px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmt(bal,S,cur.code)}</div>{Math.abs(bal)>=10000&&<div style={{fontSize:"10px",opacity:.65,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums",marginTop:"2px"}}>~{fmtCompact(bal,S,cur.code)}</div>}</div>
          ...
          <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"13px"}}>
            ...
            {(()=>{
              ...
              return<>
                {groups.map(g=>(
                  <div key={g.catLabel} style={{marginBottom:"4px"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",padding:"8px 6px",borderRadius:"8px",background:C.bg,marginBottom:"2px"}}>
                      ...
                      <span style={{fontSize:"12px",fontWeight:"700",color:C.red,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>-{fmt(g.total,S,cur.code)}</span>
                    </div>
                    {/* Individual items */}
                    {g.items.map(e=>(
                      <div key={e.id} className="hr" style={{display:"flex",alignItems:"center",gap:"9px",padding:"6px 6px 6px 18px",borderRadius:"6px",marginBottom:"1px"}}>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:"12px",color:C.inkMid,wordBreak:"break-word",whiteSpace:"normal",lineHeight:"1.4"}}>{e.note||e.catLabel}</div>
                          {recurringExp.some(r=>r.catId===e.catId)&&<div style={{fontSize:"9px",color:C.blue,fontFamily:"'DM Mono',monospace",letterSpacing:"0.05em",marginTop:"1px"}}>↻ recurring</div>}
                        </div>
                        <div style={{fontSize:"11px",color:C.inkMid,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums",flexShrink:0}}>-{fmt(e.amount,S,cur.code)}</div>
                        <button onClick={()=>{const cat=EXPENSE_CATS.find(c=>c.id===e.catId)||{id:e.catId,label:e.catLabel};recurringExp.some(r=>r.catId===e.catId)?removeRecurring(e.catId):saveRecurring(cat,e.amount);}} title={recurringExp.some(r=>r.catId===e.catId)?"Remove recurring":"Set as recurring"} style={{background:"none",border:"none",color:recurringExp.some(r=>r.catId===e.catId)?C.blue:C.borderMid,cursor:"pointer",fontSize:"13px",padding:"2px 3px",flexShrink:0}}>↻</button>
                ...
                <div style={{marginTop:"9px",paddingTop:"8px",borderTop:"1px solid "+C.border}}>{[{l:"Total Spending",v:fmt(totExp,S,cur.code),c:C.red},{l:"Saved (intentional)",v:fmt(intentionalSavings,S,cur.code),c:C.green,show:intentionalSavings>0},{l:"Net Balance",v:fmt(bal,S,cur.code),c:bal>=0?C.green:C.red,bold:true}].filter(x=>x.show!==false).map(x=>(<div key={x.l} style={{display:"flex",justifyContent:"space-between",padding:"3px 0"}}><span style={{fontSize:x.bold?"13px":"11px",color:x.bold?C.ink:C.inkMid,fontWeight:x.bold?"600":"400"}}>{x.l}</span><span style={{fontSize:x.bold?"15px":"12px",fontWeight:"700",color:x.c,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{x.v}</span></div>))}</div>
              </>;
            })()}
          </div>
          {/* ── SPENDING BREAKDOWN SUB-TABS ── */}
          {(md.expenses||[]).length>0&&<>
            <div style={{display:"flex",gap:"6px",marginTop:"14px",marginBottom:"12px",background:C.input,borderRadius:"12px",padding:"4px"}}>
              {[{id:"overview",label:"Overview"},{id:"breakdown",label:"Breakdown"},{id:"patterns",label:"Patterns"}].map(v=>(
                <button key={v.id} onClick={()=>setSpendView(v.id)} style={{flex:1,padding:"8px 6px",borderRadius:"9px",border:"none",background:spendView===v.id?C.white:"transparent",color:spendView===v.id?C.ink:C.inkLo,cursor:"pointer",fontFamily:"inherit",fontSize:"12px",fontWeight:spendView===v.id?"600":"400",boxShadow:spendView===v.id?"0 1px 4px rgba(0,0,0,0.08)":"none",transition:"all 0.15s"}}>{v.label}</button>
              ))}
            </div>

            {/* ── BREAKDOWN VIEW — pie chart + category list ── */}
            {spendView==="breakdown"&&expCatBreakdown.length>0&&<div className="scaleIn">
              <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"16px",padding:"16px",marginBottom:"10px"}}>
                <div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1.5px",marginBottom:"14px"}}>WHERE YOUR MONEY GOES</div>
                {/* Donut chart */}
                <div style={{display:"flex",alignItems:"center",gap:"14px",marginBottom:"16px"}}>
                  <div style={{width:"160px",height:"160px",flexShrink:0}}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={expCatBreakdown} cx="50%" cy="50%" innerRadius={48} outerRadius={72} dataKey="amount" paddingAngle={2}
                          label={({cx,cy,midAngle,innerRadius,outerRadius,percent})=>{
                            ...
                            return<text x={x} y={y} fill="#fff" textAnchor="middle" dominantBaseline="central" fontSize="9" fontWeight="700">{Math.round(percent*100)}%</text>;
                          }} labelLine={false}>
                          {expCatBreakdown.map((e,i)=><Cell key={i} fill={e.color} stroke="none"/>)}
                        </Pie>
                        <Tooltip formatter={(v,name)=>[fmt(v,S,cur.code),name]} contentStyle={{background:C.white,border:"1px solid "+C.border,borderRadius:"10px",fontSize:"12px"}}/>
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    {expCatBreakdown.slice(0,5).map(d=>(
                      <div key={d.catId} style={{marginBottom:"8px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:"3px"}}>
                          <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                            <div style={{width:"8px",height:"8px",borderRadius:"2px",background:d.color,flexShrink:0}}/>
                            <span style={{fontSize:"11px",color:C.inkMid,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"80px"}}>{d.label}</span>
                          </div>
                          <span style={{fontSize:"11px",fontWeight:"600",color:d.color,fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums"}}>{d.pct.toFixed(0)}%</span>
                        </div>
                        <div style={{background:C.bg,borderRadius:"99px",height:"3px",overflow:"hidden"}}>
                          <div style={{height:"100%",width:d.pct+"%",background:d.color,borderRadius:"99px",transition:"width 0.5s ease"}}/>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* Full list */}
                {expCatBreakdown.map((d,i)=>(
                  <div key={d.catId} style={{display:"flex",alignItems:"center",gap:"10px",padding:"9px 0",borderTop:"1px solid "+C.border}}>
                    <div style={{width:"10px",height:"10px",borderRadius:"3px",background:d.color,flexShrink:0}}/>
                    <span style={{flex:1,fontSize:"13px",color:C.ink,fontWeight:"500"}}>{d.label}</span>
                    <span style={{fontSize:"12px",color:C.inkLo,fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums"}}>{d.pct.toFixed(1)}%</span>
                    <span style={{fontSize:"13px",fontWeight:"700",color:C.ink,fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums",minWidth:"70px",textAlign:"right"}}>{fmt(d.amount,S,cur.code)}</span>
                  </div>
                ))}
              </div>
              {/* Smart cut suggestions */}
              {md.income>0&&<div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"16px",padding:"16px"}}>
                <div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1.5px",marginBottom:"12px"}}>SMART CUT IDEAS</div>
                {expCatBreakdown.filter(d=>d.pct>15&&d.catId!=="rent"&&d.catId!=="emi").slice(0,3).map(d=>{
                  const cut10=d.amount*0.10;
                  const annualSaving=cut10*12;
                  return<div key={d.catId} style={{background:C.bg,borderRadius:"12px",padding:"11px 13px",marginBottom:"8px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px"}}>
                      <div style={{width:"8px",height:"8px",borderRadius:"2px",background:d.color,flexShrink:0}}/>
                      <span style={{fontSize:"13px",fontWeight:"600",color:C.ink,flex:1}}>{d.label}</span>
                      <span style={{fontSize:"11px",color:C.amber,fontFamily:"'DM Mono',monospace",fontWeight:"600"}}>{d.pct.toFixed(0)}% of spend</span>
                    </div>
                    <div style={{fontSize:"12px",color:C.inkMid,lineHeight:1.5}}>{"Cut by 10% → save "+fmt(cut10,S,cur.code)+"/month · "+fmt(annualSaving,S,cur.code)+"/year"}</div>
                  </div>;
                })}
                {expCatBreakdown.filter(d=>d.pct>15&&d.catId!=="rent"&&d.catId!=="emi").length===0&&<div style={{fontSize:"13px",color:C.inkMid,textAlign:"center",padding:"8px 0"}}>Your spending looks well distributed. No obvious cuts found.</div>}
              </div>}
            </div>}

            {/* ── PATTERNS VIEW — day of week spending ── */}
            {spendView==="patterns"&&<div className="scaleIn">
              <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"16px",padding:"16px",marginBottom:"10px"}}>
                <div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1.5px",marginBottom:"4px"}}>SPENDING BY DAY OF WEEK</div>
                <div style={{fontSize:"11px",color:C.inkLo,marginBottom:"16px",lineHeight:1.5}}>Based on when you logged expenses — across all months.</div>
                {spendingByDay.totals.every(v=>v===0)
                  ?<div style={{textAlign:"center",padding:"20px",color:C.inkLo,fontSize:"13px"}}>Not enough data yet. Keep logging expenses to see patterns.</div>
                  :<>
                    <div style={{display:"flex",alignItems:"flex-end",gap:"6px",height:"80px",marginBottom:"10px"}}>
                      {spendingByDay.days.map((day,i)=>{
                        const h=spendingByDay.totals[i];
                        const barH=h>0?Math.max(8,Math.round((h/spendingByDay.maxAmt)*72)):4;
                        const isHigh=i===spendingByDay.highestDay;
                        return<div key={day} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:"4px"}}>
                          <div style={{width:"100%",height:barH+"px",borderRadius:"6px 6px 3px 3px",background:isHigh?"linear-gradient(180deg,"+C.blue+","+C.blueMid+")":C.input,transition:"height 0.4s ease",boxShadow:isHigh?"0 2px 8px "+C.blue+"44":"none"}}/>
                          <span style={{fontSize:"9px",color:isHigh?C.blue:C.inkLo,fontFamily:"'DM Mono',monospace",fontWeight:isHigh?"700":"400"}}>{day}</span>
                        </div>;
                      })}
                    </div>
                    {spendingByDay.totals[spendingByDay.highestDay]>0&&<>
                      <div style={{background:C.blueSoft,border:"1px solid "+C.blue+"22",borderRadius:"10px",padding:"10px 12px",marginBottom:"8px"}}>
                        <div style={{fontSize:"12px",color:C.blue,fontWeight:"600",marginBottom:"2px"}}>{"Highest: "+spendingByDay.days[spendingByDay.highestDay]+"s"}</div>
                        <div style={{fontSize:"12px",color:C.inkMid}}>{"You tend to spend the most on "+spendingByDay.days[spendingByDay.highestDay]+"s — "+fmt(spendingByDay.totals[spendingByDay.highestDay],S,cur.code)+" logged on "+spendingByDay.days[spendingByDay.highestDay]+"s total."}</div>
                      </div>
                      <div style={{background:C.greenSoft,border:"1px solid "+C.green+"22",borderRadius:"10px",padding:"10px 12px"}}>
                        <div style={{fontSize:"12px",color:C.green,fontWeight:"600",marginBottom:"2px"}}>{"Lowest active: "+spendingByDay.days[spendingByDay.lowestActiveDay]+"s"}</div>
                        <div style={{fontSize:"12px",color:C.inkMid}}>{"You spend the least on "+spendingByDay.days[spendingByDay.lowestActiveDay]+"s. Consider moving big purchases to this day."}</div>
                      </div>
                    </>}
                    {/* Per-day totals */}
                    <div style={{marginTop:"12px"}}>
                      {spendingByDay.days.map((day,i)=>spendingByDay.totals[i]>0&&(
                        <div key={day} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderTop:"1px solid "+C.border}}>
                          <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                            <div style={{width:"6px",height:"6px",borderRadius:"50%",background:i===spendingByDay.highestDay?C.blue:C.borderMid,flexShrink:0}}/>
                            <span style={{fontSize:"13px",color:C.ink,fontWeight:i===spendingByDay.highestDay?"600":"400"}}>{day}</span>
                            <span style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace"}}>{spendingByDay.counts[i]} entries</span>
                          </div>
                          <span style={{fontSize:"13px",fontWeight:"700",color:i===spendingByDay.highestDay?C.blue:C.ink,fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmt(spendingByDay.totals[i],S,cur.code)}</span>
                        </div>
                      ))}
                    </div>
                  </>
        ...
        {tab==="Invest"&&<div className="fade">
          ...
          <div className="hc" style={{background:"linear-gradient(135deg,#0F172A,#1E3A5F)",borderRadius:"16px",padding:"16px",marginBottom:"9px",color:"#fff",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{marginBottom:"3px"}}><div style={{fontSize:"26px",fontWeight:"800",letterSpacing:"-1px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmt(totCur,S,cur.code)}</div>{totCur>=10000&&<div style={{fontSize:"10px",opacity:.5,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums",marginTop:"1px"}}>~{fmtCompact(totCur,S,cur.code)}</div>}</div>
            <div style={{display:"flex",alignItems:"center",gap:"7px",marginBottom:"12px"}}><span style={{fontSize:"10px",color:"rgba(255,255,255,0.45)",fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>Invested: {fmt(totInv,S,cur.code)}</span><span style={{fontSize:"11px",fontWeight:"700",color:totPnL>=0?"#34D399":"#F87171",background:"rgba(255,255,255,0.08)",padding:"2px 7px",borderRadius:"20px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{totPnL>=0?"+":" "}{fmt(totPnL,S,cur.code)} ({totPct}%)</span></div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"7px"}}>{[{l:"Invested",v:fmt(totInv,S,cur.code)},{l:"Current",v:fmt(totCur,S,cur.code)},{l:"P & L",v:(totPnL>=0?"+":"")+totPct+"%",c:totPnL>=0?"#34D399":"#F87171"}].map(x=>(<div key={x.l} style={{background:"rgba(255,255,255,0.07)",borderRadius:"9px",padding:"7px 8px"}}><div style={{fontSize:"8px",opacity:.5,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"2px"}}>{x.l}</div><div style={{fontSize:"12px",fontWeight:"700",fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums",color:x.c||"#fff"}}>{x.v}</div></div>))}</div>
          </div>
                ...
                {pieData.map(d=>{
                  ...
                  return(
                    <div key={d.id} style={{display:"flex",alignItems:"center",gap:"7px",marginBottom:"8px"}}>
                      ...
                      <div style={{textAlign:"right",flexShrink:0}}>
                        <div style={{fontSize:"12px",fontWeight:"700",color:d.color,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{p2}%</div>
                        <div style={{fontSize:"9px",color:C.inkLo,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmtCompact(d.value,S,cur.code)}</div>
                      </div>
                    </div>
                  );
                })}
        ...
        {tab==="Goals"&&<div className="fade">
          ...
          :goals.map(g=>{const p=Math.min(100,g.target>0?(g.saved/g.target)*100:0);return<div key={g.id} className="hc" style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"15px",marginBottom:"9px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"10px"}}><div><div style={{fontSize:"10px",color:C.blue,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"2px"}}>{g.label.toUpperCase()}</div><div style={{fontSize:"15px",fontWeight:"700",color:C.ink}}>{g.name}</div>{g.deadline&&<div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",marginTop:"1px"}}>Target: {g.deadline}</div>}
                ...
                })()}</div><div style={{textAlign:"right"}}><div style={{fontSize:"15px",fontWeight:"700",color:C.ink,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmt(g.target,S,cur.code)}</div>{g.target>=10000&&<div style={{fontSize:"9px",color:C.inkLo,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>~{fmtCompact(g.target,S,cur.code)}</div>}<button onClick={()=>delGoal(g.id)} style={{background:"none",border:"none",color:C.inkLo,cursor:"pointer",fontSize:"11px",marginTop:"3px"}}>Remove</button></div></div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",marginBottom:"12px"}}>{[{l:"SAVED",v:fmt(g.saved,S,cur.code),c:p>=100?C.green:C.blue,a:"left"},{l:"PROGRESS",v:p.toFixed(1)+"%",c:C.ink,a:"center"},{l:"REMAINING",v:g.saved<g.target?fmt(g.target-g.saved,S,cur.code):"Done!",c:g.saved<g.target?C.red:C.green,a:"right"}].map(x=>(<div key={x.l} style={{textAlign:x.a}}><div style={{fontSize:"9px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"2px"}}>{x.l}</div><div style={{fontSize:"16px",fontWeight:"700",color:x.c,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{x.v}</div></div>))}</div>
        ...
        {tab==="Score"&&<div className="fade">
          ...
          {/* ── NET WORTH CARD ── */}
          <div style={{background:"linear-gradient(135deg,#0f0f1a,#1a1040,#0f172a)",borderRadius:"16px",padding:"18px",marginBottom:"10px",color:"#fff",position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:"-30px",right:"-30px",width:"120px",height:"120px",borderRadius:"50%",background:"rgba(99,102,241,0.12)"}}/>
            <div style={{position:"absolute",bottom:"-20px",left:"30%",width:"80px",height:"80px",borderRadius:"50%",background:"rgba(79,70,229,0.08)"}}/>
            <div style={{fontSize:"9px",letterSpacing:"2px",opacity:0.5,fontFamily:"'DM Mono',monospace",marginBottom:"4px"}}>NET WORTH</div>
            <div style={{fontSize:"28px",fontWeight:"700",fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums",letterSpacing:"-0.04em",marginBottom:"12px",color:netWorth>=0?"#34D399":"#F87171"}}>{fmt(netWorth,S,cur.code)}</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px"}}>
              {[{l:"Assets",v:fmt(totCur+(goals.reduce((s,g)=>s+g.saved,0)),S,cur.code),c:"#34D399"},{l:"Liabilities",v:fmt(loans.reduce((s,l)=>s+l.remaining,0),S,cur.code),c:"#F87171"},{l:"Streak",v:savingStreak+"mo 🔥",c:"#FBBF24"}].map(x=>(
                <div key={x.l} style={{background:"rgba(255,255,255,0.06)",borderRadius:"10px",padding:"8px 10px"}}>
                  <div style={{fontSize:"8px",opacity:0.5,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"3px"}}>{x.l.toUpperCase()}</div>
                  <div style={{fontSize:"12px",fontWeight:"700",color:x.c,fontFamily:"'Inter',sans-serif",fontVariantNumeric:"tabular-nums"}}>{x.v}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {md.income>0&&<div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"14px 16px",marginBottom:"10px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
              ...
              <div style={{fontSize:"11px",color:parseFloat(savPct)>=20?C.green:parseFloat(savPct)>=0?C.amber:C.red,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums",fontWeight:"700"}}>{savPct}% saved</div>
            </div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:"6px",fontSize:"10px",color:C.inkLo,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}><span>Spent: {fmt(totExp,S,cur.code)}</span><span>Saved: {fmt(totalSaved,S,cur.code)}</span></div>
          </div>}
              ...
              return<div>
                <div style={{marginBottom:"12px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:"6px"}}>
                    ...
                    <span style={{fontSize:"12px",fontWeight:"700",color:C.blue,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmt(extraPerMonth,S,cur.code)}</span>
                  </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"7px"}}>
                  {results.map(r=>(
                    <div key={r.yrs} style={{background:C.bg,borderRadius:"10px",padding:"10px 8px",textAlign:"center"}}>
                      ...
                      <div style={{fontSize:"13px",fontWeight:"700",color:C.green,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{fmt(r.fv,S,cur.code)}</div>
                    </div>
                  ))}
                </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"9px",marginBottom:"10px"}}>
            {[{label:"Income",val:fmt(md.income||0,S,cur.code),c:C.green,sub:MONTHS_S[mo],tab:"Budget"},{label:"Invested",val:fmt(totCur,S,cur.code),c:totPnL>=0?C.green:C.red,sub:(totPnL>=0?"+":"")+totPct+"%",tab:"Invest"},{label:"Loans",val:loans.length+" active",c:C.red,sub:fmt(loans.reduce((s,l)=>s+l.remaining,0),S,cur.code)+" owed",tab:"Loans"},{label:"Goals",val:goals.length+" set",c:C.blue,sub:fmt(goals.reduce((s,g)=>s+g.saved,0),S,cur.code)+" saved",tab:"Score"}].map(x=>(
              <div key={x.label} className="hc" onClick={()=>switchTab(x.tab)} style={{background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"12px 13px",cursor:"pointer"}}>
                ...
                <div style={{fontSize:"15px",fontWeight:"700",color:x.c,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums",marginBottom:"2px"}}>{x.val}</div>
                ...
                :rows.map((b,i)=>(
                  <div key={b.bank} className="hc" style={{background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"13px",marginBottom:"7px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"9px",marginBottom:"7px"}}>
                      ...
                      <div><span style={{fontSize:"14px",fontWeight:"800",color:i===0?C.green:C.blue,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{b.min}%</span><span style={{fontSize:"10px",color:C.inkLo}}> – {b.max}%</span></div>
                    </div>
        ...
        {tab==="Tools"&&<div className="fade">
          ...
          {calcRes&&<div className="scaleIn"><div style={{background:C.blueSoft,border:"1px solid "+C.blue+"33",borderRadius:"14px",padding:"15px",textAlign:"center"}}><div style={{fontSize:"9px",color:C.blue,letterSpacing:"2px",fontFamily:"'DM Mono',monospace",marginBottom:"4px"}}>MONTHLY EMI</div><div style={{fontSize:"32px",fontWeight:"800",color:C.blue,fontFamily:"'Plus Jakarta Sans',sans-serif",letterSpacing:"-1px",fontVariantNumeric:"tabular-nums",marginBottom:"13px"}}>{fmt(calcRes.emi,S,cur.code)}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"7px",textAlign:"left"}}>{[{l:"Principal",v:fmt(calcRes.a,S,cur.code)},{l:"Total Interest",v:fmt(calcRes.interest,S,cur.code),c:C.red},{l:"Total Payable",v:fmt(calcRes.total,S,cur.code),c:C.green},{l:"Rate Used",v:calcRes.r+"% p.a."}].map(x=>(<div key={x.l} style={{background:"rgba(255,255,255,0.65)",borderRadius:"9px",padding:"8px 9px"}}><div style={{fontSize:"9px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"2px"}}>{x.l.toUpperCase()}</div><div style={{fontSize:"12px",fontWeight:"700",color:x.c||C.ink,fontFamily:"'Plus Jakarta Sans',sans-serif",fontVariantNumeric:"tabular-nums"}}>{x.v}</div></div>))}</div></div></div>}
        </div>}
      ...
      {modal&&<div onClick={()=>setModal(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",backdropFilter:"blur(12px)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
        <div onClick={e=>e.stopPropagation()} style={{background:C.white,borderRadius:"24px 24px 0 0",padding:"20px 18px 44px",width:"100%",maxWidth:"640px",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 -8px 48px rgba(0,0,0,0.15)",animation:"slideDown 0.25s cubic-bezier(0.16,1,0.3,1)"}}>
          <div style={{width:"36px",height:"3px",background:C.borderMid,borderRadius:"99px",margin:"0 auto 20px"}}/>
