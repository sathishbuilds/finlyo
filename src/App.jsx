    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter','Plus Jakarta Sans',sans-serif",color:C.ink,overflowX:"hidden",maxWidth:"100vw"}}>
      ...
      <nav className="tabbar" style={{background:darkMode?"rgba(9,9,15,0.92)":"rgba(255,255,255,0.92)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid "+C.border,padding:"0 12px",display:"flex",overflowX:"auto",scrollbarWidth:"none",gap:"2px"}}>
        ...
        <div style={{position:"relative",marginLeft:"auto",flexShrink:0}} onClick={e=>e.stopPropagation()}>
          <button onClick={()=>setShowMore(s=>!s)} style={{padding:"11px 15px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",fontWeight:["Tools","Rates","Goals"].includes(tab)?"600":"400",color:["Tools","Rates","Goals"].includes(tab)?C.blue:C.inkLo,borderBottom:["Tools","Rates","Goals"].includes(tab)?"2px solid "+C.blue:"2px solid transparent",whiteSpace:"nowrap",letterSpacing:"-0.01em",transition:"color 0.15s"}}>
            Tools ▾
          </button>
          {showMore&&(
            <div style={{position:"absolute",right:0,top:"42px",background:C.white,border:"1px solid "+C.border,borderRadius:"14px",overflow:"hidden",zIndex:500,minWidth:"160px",boxShadow:"0 12px 40px rgba(0,0,0,0.16)",animation:"scaleIn 0.15s ease"}}>
              {[{label:"Rates",value:"Rates",icon:"📊"},{label:"EMI Calc",value:"Tools",icon:"🧮"},{label:"Goals",value:"Goals",icon:"🎯"}].map(item=>(
                <button key={item.value} onClick={()=>{switchTab(item.value);}} style={{display:"flex",alignItems:"center",gap:"10px",width:"100%",padding:"12px 16px",border:"none",background:tab===item.value?C.blueSoft:"transparent",color:tab===item.value?C.blue:C.ink,cursor:"pointer",fontFamily:"inherit",fontSize:"13px",textAlign:"left",borderBottom:"1px solid "+C.border,fontWeight:tab===item.value?"600":"400",transition:"background 0.1s"}}><span style={{fontSize:"14px"}}>{item.icon}</span>{item.label}</button>
              ))}
            </div>
          )}
        </div>
