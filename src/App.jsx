...
const PRIMARY_TABS=["Clarity","Your Money","Your Progress","Your Plan"];
const SECONDARY_TABS=["Milestones","Rates","Calculator","Learn"];
const ARTICLES=[{id:1,tag:"Essential",title:"How to Use Your Credit Card Without Getting Burned",summary:"A credit card is one of the sharpest tools in finance. Used correctly, it builds wealth.",body:"Pay the full balance every month. Carrying a balance means paying 18-45% annual interest.\n\n**Only spend what you already have**\nYour credit card is a 30-day payment bridge, not a loan.\n\n**Keep utilisation below 30%**\nOn a Rs.1,00,000 limit, never let your balance exceed Rs.30,000.\n\n**Never miss a due date**\nOne missed payment drops your score 80-100 points and stays on your report for 36 months."},{id:2,tag:"Must Read",title:"Your CIBIL Score — The Number That Precedes You",summary:"Before you walk into any bank, this number has already spoken for you.",body:"Your CIBIL score is between 300 and 900.\n\n**What it means in money**\nOn a Rs.50L home loan: 780+ score gets 8.5%. A score of 680 gets 9.5%. That 1% gap costs Rs.7-8 lakh more total.\n\n**The five factors**\n1. Payment history (35%) - Never miss a payment.\n2. Credit utilisation (30%) - Stay below 30% of limit.\n3. Length of history (15%) - Never close your oldest card.\n4. Credit mix (10%) - Have both secured and unsecured credit.\n5. New enquiries (10%) - Do not apply for multiple loans at once."},{id:3,tag:"Strategy",title:"Good Debt vs Bad Debt — Not All Borrowing Is Equal",summary:"The wealthy borrow constantly. The financially stuck borrow desperately.",body:"**Good debt creates more than it costs**\nA home loan funds an appreciating asset with tax benefits.\n\n**Bad debt drains quietly**\nPersonal loans for weddings fund consumption at 12-24% interest.\n\n**The prepayment insight**\nOne extra EMI per year on a 20-year home loan cuts tenure by 3-4 years."},{id:4,tag:"Awareness",title:"Seven Habits That Keep People Financially Stuck",summary:"Not mistakes made once. Patterns repeated across years.",body:"**1. No emergency fund** — Every disruption becomes debt.\n**2. Investing while carrying expensive debt** — 36% card vs 12% fund = 24% net loss.\n**3. Spending every raise** — Wealth lives in the gap.\n**4. Treating insurance as optional** — One hospitalisation can erase five years of savings.\n**5. Not starting small** — Rs.2,000/month at 12% from 25 = Rs.1.05Cr by 60.\n**6. Panic-selling** — Markets always recover.\n**7. Not tracking** — People who track save 15-20% more."},{id:5,tag:"Advanced",title:"Using Leverage to Build Wealth",summary:"Leverage is not risk. Misapplied leverage is risk.",body:"**Real estate leverage**\nBuy Rs.70L with Rs.17.5L down. If it appreciates 9%, you gain Rs.6.3L on Rs.17.5L — 36% ROI.\n\n**Three conditions for safe leverage**\n1. Cash flow covers 1.5x EMI.\n2. Asset appreciates or generates income.\n3. Maintain 6-month EMI buffer in cash."},{id:6,tag:"System",title:"The 50/30/20 Rule",summary:"Three numbers. One rule. Applied consistently, it builds genuine financial freedom.",body:"**50% — Essentials**: Rent, EMIs, groceries, insurance.\n**30% — Living well**: Dining, entertainment, travel.\n**20% — Your future**: Emergency fund, then SIPs, PPF, NPS.\n\n**The automation principle**\nAuto-transfer 20% on salary day before spending begins."}];
...
function calcHealthScore(income,expenses,loans,goals,invests){
  if(!income||income===0)return{score:0,savings:0,spending:0,debt:0,growth:0,savPct:"0.0",spendRatio:"0.0"};
  ...
  const debtRatio=income>0?Math.min(1,totLoan/(income*12)):0;
  ...
  return{score:Math.round(savScore+spendScore+debtScore+growthScore),savings:Math.round(savScore),spending:Math.round(spendScore),debt:Math.round(debtScore),growth:Math.round(growthScore),savPct:savPct.toFixed(1),spendRatio:(spendRatio*100).toFixed(1)};
}
...
function scoreLabel(s){
  ...
  return{label:"Start Here",color:"#C0312B",msg:"Add your income to get your full clarity picture."};
}

function genInsights(income,curExp,prevExp,loans,savPct){
  const ins=[];
  const curTotal=curExp.reduce((s,e)=>s+e.amount,0);
  const prevTotal=prevExp.reduce((s,e)=>s+e.amount,0);
  if(prevTotal>0&&curTotal>0){const diff=curTotal-prevTotal;const pct=Math.abs((diff/prevTotal)*100).toFixed(0);if(diff<0)ins.push({type:"positive",icon:"↓",text:"You spent "+pct+"% less than last month. Nice discipline."});else if(diff>0)ins.push({type:"warning",icon:"↑",text:"Spending is up "+pct+"% from last month. Worth reviewing."});else ins.push({type:"neutral",icon:"→",text:"Spending is consistent with last month."});}
  if(income>0){const sp=(curTotal/income)*100;if(sp<=50)ins.push({type:"positive",icon:"✓",text:"You're on track this month. Spending is healthy."});else if(sp<=70)ins.push({type:"warning",icon:"!",text:"Spending is getting high. Watch your discretionary costs."});else ins.push({type:"negative",icon:"↑",text:"You're overspending this month. Review your budget."});}
  if(curExp.length>0){const catMap={};curExp.forEach(e=>{catMap[e.catLabel]=(catMap[e.catLabel]||0)+e.amount;});const top=Object.entries(catMap).sort((a,b)=>b[1]-a[1])[0];if(top)ins.push({type:"neutral",icon:"◎",text:top[0]+" is your highest expense this month."});}
  const sav=parseFloat(savPct);
  if(income>0){if(sav>=30)ins.push({type:"positive",icon:"★",text:"Saving "+sav+"% of income. Above the 30% target. Well done."});else if(sav>=20)ins.push({type:"positive",icon:"★",text:"Saving "+sav+"% of income. On target. Try pushing to 30%."});else if(sav>=0)ins.push({type:"warning",icon:"!",text:"Saving only "+sav+"% of income. Target is 20%. Reduce non-essentials."});else ins.push({type:"negative",icon:"↓",text:"You're spending more than you earn this month."});}
...
function HealthScore({score,breakdown,C}){
  ...
  return(
    <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"16px",padding:"18px",marginBottom:"10px"}}>
      ...
      <div style={{background:C.bg,borderRadius:"10px",padding:"10px 12px",fontSize:"12px",color:C.inkMid,fontStyle:"italic"}}>{msg}</div>
    </div>
  );
}
...
function Auth({darkMode,onToggleTheme,onLogin}){
  ...
  const doRegister=async()=>{setErr("");setMsg("");if(!valid(email))return setErr("Please enter a valid email address.");if(pass.length<8)return setErr("Password must be at least 8 characters.");if(pass!==confirm)return setErr("Passwords do not match.");if(!agreed)return setErr("Please accept the Terms of Use to continue.");setLoading(true);const{error}=await supabase.auth.signUp({email,password:pass});setLoading(false);if(error)return setErr(error.message);setMsg("Account created! You can now sign in.");setScreen("login");};
    ...
    <div style={{background:C.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",fontFamily:"'Plus Jakarta Sans','Inter',sans-serif",color:C.ink}}>
      ...
      <div style={{width:"100%",maxWidth:"400px"}}>
        ...
        <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:"18px",padding:"24px 20px",boxShadow:"0 4px 32px rgba(0,0,0,0.08)"}}>
          ...
          {screen==="register"&&<div style={{display:"flex",alignItems:"flex-start",gap:"10px",padding:"11px 12px",background:C.input,borderRadius:"10px",border:"1px solid "+C.border,marginBottom:"4px"}}><button onClick={()=>setAgreed(a=>!a)} style={{width:"18px",height:"18px",borderRadius:"5px",border:"2px solid "+(agreed?"#1B5FD4":C.border),background:agreed?"#1B5FD4":"transparent",cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",marginTop:"1px"}}>{agreed&&<div style={{width:"8px",height:"8px",borderRadius:"2px",background:"#fff"}}/>}</button><div style={{fontSize:"12px",color:C.inkMid,lineHeight:1.5}}>I agree to the <button onClick={()=>setShowTerms(true)} style={{background:"none",border:"none",color:"#1B5FD4",cursor:"pointer",fontFamily:"inherit",fontSize:"12px",fontWeight:"600",textDecoration:"underline"}}>Terms of Use and Privacy Policy</button></div></div>}
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Plus Jakarta Sans','Inter',sans-serif",color:C.ink}}>
      ...
      <div style={{padding:"20px 16px",maxWidth:"480px",margin:"0 auto"}}>
        ...
        <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"16px",marginBottom:"10px"}}><div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1.5px",marginBottom:"10px"}}>ACCOUNT</div><div style={{fontSize:"14px",fontWeight:"600",color:C.ink}}>{user?.email}</div><div style={{fontSize:"11px",color:C.inkLo,fontFamily:"'DM Mono',monospace",marginTop:"3px"}}>Member · Finlyo Money Clarity System</div></div>
        <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"16px",marginBottom:"10px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div><div style={{fontSize:"14px",fontWeight:"600",color:C.ink}}>{darkMode?"Dark Mode":"Light Mode"}</div><div style={{fontSize:"11px",color:C.inkLo,marginTop:"2px"}}>Switch between light and dark</div></div><button onClick={onToggleTheme} style={{width:"52px",height:"28px",borderRadius:"100px",border:"none",background:darkMode?C.blue:"#CBD5E1",cursor:"pointer",position:"relative",transition:"background .25s",flexShrink:0}}><div style={{position:"absolute",top:"3px",left:darkMode?"27px":"3px",width:"22px",height:"22px",borderRadius:"50%",background:"#fff",transition:"left .25s",boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}/></button></div>
        </div>
        <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"16px",marginBottom:"10px"}}><div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"1.5px",marginBottom:"10px"}}>SESSION</div><button onClick={async()=>{await supabase.auth.signOut();onSignOut();}} style={{width:"100%",padding:"11px",borderRadius:"10px",border:"1px solid "+C.border,background:"transparent",color:C.inkMid,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontWeight:"600",fontSize:"11px",letterSpacing:"2px"}}>SIGN OUT</button></div>
        <div style={{background:C.white,border:"1px solid "+C.red+"33",borderRadius:"14px",padding:"16px",marginBottom:"10px"}}>
          ...
          <div style={{marginBottom:"16px"}}><div style={{fontSize:"13px",fontWeight:"600",color:C.ink,marginBottom:"4px"}}>Delete all financial data</div><div style={{fontSize:"12px",color:C.inkMid,marginBottom:"10px",lineHeight:1.5}}>Removes all budgets, goals, loans, and investments. Your account stays.</div><input value={confirm} onChange={e=>{setConfirm(e.target.value);setErr("");}} placeholder='Type "delete my data" to confirm' style={inp}/><button onClick={doDeleteData} style={{width:"100%",padding:"11px",borderRadius:"10px",border:"none",background:C.red,color:"#fff",cursor:"pointer",fontFamily:"'DM Mono',monospace",fontWeight:"600",fontSize:"11px",letterSpacing:"2px"}}>DELETE ALL DATA</button></div>
          <div style={{borderTop:"1px solid "+C.border,paddingTop:"16px"}}><div style={{fontSize:"13px",fontWeight:"600",color:C.ink,marginBottom:"4px"}}>Delete account</div><div style={{fontSize:"12px",color:C.inkMid,marginBottom:"10px",lineHeight:1.5}}>Deletes all your data and signs you out permanently.</div><input value={confirm} onChange={e=>{setConfirm(e.target.value);setErr("");}} placeholder='Type "delete my account" to confirm' style={inp}/><button onClick={doDeleteAccount} style={{width:"100%",padding:"11px",borderRadius:"10px",border:"none",background:"#7F1D1D",color:"#fff",cursor:"pointer",fontFamily:"'DM Mono',monospace",fontWeight:"600",fontSize:"11px",letterSpacing:"2px"}}>DELETE ACCOUNT</button></div>
        </div>
        <div style={{textAlign:"center",marginTop:"14px"}}><button onClick={()=>setShowTerms(true)} style={{background:"none",border:"none",color:C.inkLo,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"10px",letterSpacing:"1px",textDecoration:"underline"}}>VIEW TERMS AND PRIVACY POLICY</button></div>
      </div>
      {showTerms&&<TermsModal onClose={()=>setShowTerms(false)} C={C}/>}
    </div>
  );
...
function Finlyo({user,darkMode,onOpenSettings}){
  ...
  const insights=genInsights(md.income,md.expenses||[],prevMd.expenses||[],loans,savPct);
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      setBudgets(bMap);setGoals(g.data||[]);setLoans((l.data||[]).map(loan=>({...loan,paidKeys:loan.paid_keys||[],remMonths:loan.rem_months,prePaid:loan.pre_paid||0})));setInvests(iv.data||[]);setLoading(false);
    })();
  },[uid]);

  const showToast=(msg,isInvest=false)=>{setToast(msg);if(isInvest){setRain(true);clearTimeout(rainRef.current);rainRef.current=setTimeout(()=>setRain(false),3000);}clearTimeout(toastRef.current);toastRef.current=setTimeout(()=>setToast(null),2800);};
  ...
  const css="@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&family=DM+Mono:wght@300;400;500&display=swap');"+"*{box-sizing:border-box;margin:0;padding:0;}"+"body{background:"+C.bg+";}"
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Plus Jakarta Sans','Inter',sans-serif",color:C.ink,overflowX:"hidden",maxWidth:"100vw"}}>
      ...
      <nav className="tabbar" style={{background:C.white,borderBottom:"1px solid "+C.border,padding:"0 10px",display:"flex",overflowX:"auto",scrollbarWidth:"none",position:"relative"}}>
        {PRIMARY_TABS.map(t=><button key={t} onClick={()=>switchTab(t)} style={{flexShrink:0,padding:"10px 12px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:"12px",fontWeight:tab===t?"700":"400",color:tab===t?C.blue:C.inkLo,borderBottom:tab===t?"2px solid "+C.blue:"2px solid transparent",letterSpacing:"-.1px",whiteSpace:"nowrap",transition:"color .15s"}}>{t}</button>)}
        <div style={{position:"relative",marginLeft:"auto",flexShrink:0}}>
          <button onClick={()=>setShowMore(!showMore)} style={{padding:"10px 12px",border:"none",background:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:"12px",fontWeight:SECONDARY_TABS.includes(tab)?"700":"400",color:SECONDARY_TABS.includes(tab)?C.blue:C.inkLo,borderBottom:SECONDARY_TABS.includes(tab)?"2px solid "+C.blue:"2px solid transparent",whiteSpace:"nowrap"}}>{SECONDARY_TABS.includes(tab)?tab:"More"} &#9660;</button>
          {showMore&&<div style={{position:"absolute",right:0,top:"42px",background:C.white,border:"1px solid "+C.border,borderRadius:"12px",overflow:"hidden",zIndex:200,width:"160px",boxShadow:"0 8px 32px rgba(0,0,0,0.12)"}}>{SECONDARY_TABS.map(t=><button key={t} onClick={()=>switchTab(t)} style={{display:"block",width:"100%",padding:"10px 14px",border:"none",background:tab===t?C.blueSoft:"transparent",color:tab===t?C.blue:C.ink,cursor:"pointer",fontFamily:"inherit",fontSize:"13px",textAlign:"left",borderBottom:"1px solid "+C.border}}>{t}</button>)}</div>}
        </div>
      </nav>
      ...
      <main style={{padding:"14px 13px 56px",maxWidth:"640px",margin:"0 auto"}} onClick={()=>{showCur&&setShowCur(false);showMore&&setShowMore(false);}}>
        ...
        {tab==="Clarity"&&<div className="fade">
          ...
          {md.income>0&&<div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"14px 16px",marginBottom:"10px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
              <div style={{fontSize:"12px",fontWeight:"600",color:C.ink}}>{parseFloat(savPct)>=20?"You're on track this month.":parseFloat(savPct)>=0?"Spending is getting high.":"You're overspending this month."}</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"9px",marginBottom:"10px"}}>
            {[{label:"Income",val:fmt(md.income||0,S),c:C.green,sub:MONTHS_S[mo]},{label:"Spent",val:fmt(totExp,S),c:C.red,sub:Object.keys(catTotals).length+" categories"},{label:"Goals",val:goals.length+" active",c:C.blue,sub:goals.length>0?fmt(goals.reduce((s,g)=>s+g.saved,0),S)+" saved":"Set your first goal"},{label:"Portfolio",val:fmt(totCur,S),c:totPnL>=0?C.green:C.red,sub:(totPnL>=0?"+":"")+totPct+"%"}].map(x=>(
              <div key={x.label} className="hc" style={{background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"12px 13px"}}>
          ...
          {!md.income&&<div style={{background:C.blueSoft,border:"1px solid "+C.blue+"33",borderRadius:"12px",padding:"13px 14px",textAlign:"center"}}><div style={{fontSize:"13px",color:C.blue,fontWeight:"600",marginBottom:"4px"}}>Start with your income</div><div style={{fontSize:"12px",color:C.inkMid,marginBottom:"10px"}}>Add this month's income to unlock your full clarity picture.</div><button onClick={()=>switchTab("Your Money")} style={{padding:"8px 18px",borderRadius:"20px",border:"none",background:C.blue,color:"#fff",cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"11px",letterSpacing:"1px",fontWeight:"600"}}>ADD INCOME</button></div>}
        </div>}

        {tab==="Your Money"&&<div className="fade">
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"14px"}}>
            <button onClick={prevMo} style={{width:"30px",height:"30px",borderRadius:"50%",border:"1px solid "+C.border,background:C.white,color:C.inkMid,cursor:"pointer",fontSize:"14px",display:"flex",alignItems:"center",justifyContent:"center"}}>&#8249;</button>
            <button onClick={()=>setShowMoPicker(true)} style={{background:"none",border:"none",cursor:"pointer",textAlign:"center"}}><div style={{fontSize:"16px",fontWeight:"700",color:C.ink}}>{MONTHS[mo]}</div><div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",letterSpacing:"2px"}}>{yr} &#9660;</div></button>
            <button onClick={nextMo} style={{width:"30px",height:"30px",borderRadius:"50%",border:"1px solid "+C.border,background:C.white,color:C.inkMid,cursor:"pointer",fontSize:"14px",display:"flex",alignItems:"center",justifyContent:"center"}}>&#8250;</button>
          </div>
          <div className="hc" style={{background:"linear-gradient(135deg,"+C.blue+","+C.blueMid+")",borderRadius:"16px",padding:"18px 16px",marginBottom:"10px",color:"#fff",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"7px"}}>
              {[{l:"Income",v:fmt(md.income||0,S)},{l:"Spent",v:fmt(totExp,S)},{l:"Saved",v:savPct+"%"}].map(x=>(
                <div key={x.l} style={{background:"rgba(255,255,255,0.12)",borderRadius:"9px",padding:"7px 8px"}}><div style={{fontSize:"8px",opacity:.65,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"2px"}}>{x.l.toUpperCase()}</div><div style={{fontSize:"12px",fontWeight:"700",fontFamily:"'DM Mono',monospace"}}>{x.v}</div></div>
              ))}
            </div>
          ...
          <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"13px"}}>
            ...
            {(md.expenses||[]).length===0?<div style={{textAlign:"center",padding:"20px 0",color:C.inkLo,fontSize:"12px"}}>No expenses yet this month.</div>:<>
              {(md.expenses||[]).map(e=>(<div key={e.id} className="hr" style={{display:"flex",alignItems:"center",gap:"9px",padding:"8px 6px",borderRadius:"8px",marginBottom:"3px"}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:"13px",fontWeight:"500",color:C.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.note||e.catLabel}</div><div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace"}}>{e.catLabel}</div></div><div style={{fontSize:"12px",fontWeight:"600",color:C.red,fontFamily:"'DM Mono',monospace",flexShrink:0}}>-{fmt(e.amount,S)}</div><button onClick={()=>delExp(e.id)} style={{background:"none",border:"none",color:C.inkLo,cursor:"pointer",fontSize:"12px",padding:"2px 3px",flexShrink:0}}>✕</button></div>))}
        ...
        {tab==="Your Progress"&&<div className="fade">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
            <div><h2 style={{fontSize:"18px",fontWeight:"800",color:C.ink,letterSpacing:"-.4px"}}>Your Progress</h2><p style={{fontSize:"11px",color:C.inkLo,marginTop:"2px"}}>Every rupee you save is a vote for your future.</p></div>
          ...
          :goals.map(g=>{const p=Math.min(100,g.target>0?(g.saved/g.target)*100:0);return<div key={g.id} className="hc" style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"15px",marginBottom:"9px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"10px"}}><div><div style={{fontSize:"10px",color:C.blue,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"2px"}}>{g.label.toUpperCase()}</div><div style={{fontSize:"15px",fontWeight:"700",color:C.ink}}>{g.name}</div>{g.deadline&&<div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",marginTop:"1px"}}>By {g.deadline}</div>}</div><div style={{textAlign:"right"}}><div style={{fontSize:"15px",fontWeight:"700",color:C.ink,fontFamily:"'DM Mono',monospace"}}>{fmt(g.target,S)}</div><button onClick={()=>delGoal(g.id)} style={{background:"none",border:"none",color:C.inkLo,cursor:"pointer",fontSize:"11px",marginTop:"3px"}}>Remove</button></div></div>
        ...
        {tab==="Your Plan"&&<div className="fade">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
            <div><h2 style={{fontSize:"18px",fontWeight:"800",color:C.ink,letterSpacing:"-.4px"}}>Your Plan</h2><p style={{fontSize:"11px",color:C.inkLo,marginTop:"2px"}}>Stay ahead of every commitment.</p></div>
        ...
        {tab==="Milestones"&&<div className="fade">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
            <div><h2 style={{fontSize:"18px",fontWeight:"800",color:C.ink,letterSpacing:"-.4px"}}>Milestones</h2><p style={{fontSize:"11px",color:C.inkLo,marginTop:"2px"}}>Wealth that works while you sleep.</p></div>
        ...
        {tab==="Rates"&&<div className="fade">
          ...
          <div style={{display:"flex",gap:"7px",marginBottom:"10px"}}>{[{k:"govt",l:"Government Banks"},{k:"private",l:"Private Banks"}].map(x=><button key={x.k} onClick={()=>setRateFilter(f=>({...f,bankType:x.k}))} style={{flex:1,padding:"9px",borderRadius:"10px",border:"1px solid "+(rateFilter.bankType===x.k?C.blue:C.border),background:rateFilter.bankType===x.k?C.blueSoft:C.white,color:rateFilter.bankType===x.k?C.blue:C.inkMid,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"11px",fontWeight:rateFilter.bankType===x.k?"700":"400"}}>{x.l}</button>)}</div>
        ...
        {tab==="Calculator"&&<div className="fade">
          ...
          <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"14px",marginBottom:"10px"}}>
            ...
            <div style={{marginBottom:"10px"}}><label style={lbl}>INTEREST RATE (% P.A.)</label><input type="number" step="0.1" value={calcForm.rate} onChange={e=>setCalcForm(f=>({...f,rate:e.target.value}))} placeholder="e.g. 8.5 — see Rates tab" style={inp}/></div>
        ...
        {tab==="Learn"&&<div className="fade">
          {article?<><button onClick={()=>setArticle(null)} style={{display:"inline-flex",alignItems:"center",gap:"5px",padding:"6px 12px",borderRadius:"20px",border:"1px solid "+C.border,background:C.white,color:C.inkMid,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"10px",letterSpacing:"1px",marginBottom:"13px"}}>&#8592; BACK</button>
            <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"18px"}}><span style={{fontSize:"10px",background:C.blueSoft,color:C.blue,padding:"2px 9px",borderRadius:"20px",fontFamily:"'DM Mono',monospace",letterSpacing:"1px"}}>{article.tag.toUpperCase()}</span><h2 style={{fontSize:"18px",fontWeight:"800",color:C.ink,lineHeight:1.3,marginTop:"9px",marginBottom:"9px",letterSpacing:"-.3px"}}>{article.title}</h2><p style={{fontSize:"13px",color:C.inkMid,fontStyle:"italic",lineHeight:1.6,paddingLeft:"11px",borderLeft:"2px solid "+C.blue+"44",marginBottom:"16px"}}>{article.summary}</p><div style={{fontSize:"13px",lineHeight:"1.85",color:C.inkMid}}>{article.body.split("\n").map((line,i)=>{if(line.startsWith("**")&&line.endsWith("**"))return<div key={i} style={{fontWeight:"700",color:C.ink,fontSize:"13px",marginTop:"16px",marginBottom:"4px"}}>{line.replace(/\*\*/g,"")}</div>;if(line.match(/^\d+\./))return<div key={i} style={{padding:"3px 0 3px 9px",borderLeft:"2px solid "+C.border,marginBottom:"3px"}}>{line}</div>;return<div key={i} style={{marginBottom:line===""?"9px":"2px"}}>{line}</div>;})}</div></div>
          </>:<>
            ...
            {ARTICLES.map(a=>(<div key={a.id} className="hc" onClick={()=>setArticle(a)} style={{background:C.white,border:"1px solid "+C.border,borderRadius:"12px",padding:"13px",marginBottom:"8px",cursor:"pointer"}}><span style={{fontSize:"9px",background:C.blueSoft,color:C.blue,padding:"2px 8px",borderRadius:"20px",fontFamily:"'DM Mono',monospace",letterSpacing:"1px"}}>{a.tag.toUpperCase()}</span><h3 style={{fontSize:"14px",fontWeight:"700",color:C.ink,lineHeight:1.35,marginTop:"7px",marginBottom:"4px",letterSpacing:"-.2px"}}>{a.title}</h3><p style={{fontSize:"11px",color:C.inkLo,lineHeight:1.5,marginBottom:"8px"}}>{a.summary}</p><div style={{fontSize:"10px",color:C.blue,fontFamily:"'DM Mono',monospace",letterSpacing:"1px"}}>READ &#8594;</div></div>))}
          </>}
        </div>}
