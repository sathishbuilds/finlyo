...
function Finlyo({user,darkMode,onOpenSettings}){
  ...
  const quickAdd=async()=>{
    ...
    if(!amt||amt<=0)return;
    const cat=EXPENSE_CATS.find(c=>c.id===quickCat);
    const newExp={id:Date.now(),catId:cat.id,catLabel:cat.label,amount:amt,note:""};
    ...
    setBudgets(p=>({...p,[mk]:{...p[mk],expenses}}));
    setQuickAmt("");setShowQuick(false);
    showToast(cat.label+" · "+S+amt+" logged.");
  };
      ...
      {showQuick&&<div onClick={()=>setShowQuick(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(8px)",zIndex:800,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
        <div onClick={e=>e.stopPropagation()} style={{background:C.white,borderRadius:"24px 24px 0 0",padding:"20px 18px 40px",width:"100%",maxWidth:"480px",boxShadow:"0 -8px 48px rgba(0,0,0,0.15)",animation:"slideDown 0.25s cubic-bezier(0.16,1,0.3,1)"}}>
          ...
          <div style={{display:"flex",gap:"6px",flexWrap:"wrap",marginBottom:"16px"}}>
            {[{id:"dining",label:"🍔 Food"},{id:"transport",label:"🚗 Travel"},{id:"shopping",label:"🛍 Shopping"},{id:"grocery",label:"🛒 Grocery"},{id:"entertain",label:"🎬 Fun"},{id:"utilities",label:"⚡ Bills"},{id:"health",label:"💊 Health"},{id:"misc",label:"📦 Other"}].map(c=>(
      ...
      <main style={{padding:"14px 13px 80px",maxWidth:"640px",margin:"0 auto"}} onClick={()=>{showCur&&setShowCur(false);showMore&&setShowMore(false);}}>
        ...
        {tab==="Budget"&&<div className="fade">
          ...
          <div style={{display:"flex",gap:"8px",marginBottom:"14px"}}>
            ...
            {savingStreak>=2&&<div style={{background:"linear-gradient(135deg,#F59E0B,#EF4444)",borderRadius:"12px",padding:"10px 13px",display:"flex",flexDirection:"column",justifyContent:"center",minWidth:"80px"}}>
              <div style={{fontSize:"18px",textAlign:"center"}}>🔥</div>
              <div style={{fontSize:"10px",color:"rgba(255,255,255,0.9)",textAlign:"center",fontWeight:"600"}}>{savingStreak} months</div>
            </div>}
          </div>
          <MonthNav/>
