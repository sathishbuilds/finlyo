      ...
      <main style={{padding:"14px 13px 56px",maxWidth:"640px",margin:"0 auto"}} onClick={()=>{showCur&&setShowCur(false);showMore&&setShowMore(false);}}>
        ...
        {tab==="Budget"&&<div className="fade">
          ...
          <div className="hc" style={{background:"linear-gradient(135deg,"+C.blue+","+C.blueMid+")",borderRadius:"16px",padding:"18px 16px",marginBottom:"10px",color:"#fff",position:"relative",overflow:"hidden"}}>
            ...
            {md.income>0&&<><div style={{marginTop:"10px",background:"rgba(255,255,255,0.2)",borderRadius:"100px",height:"3px",overflow:"hidden"}}><div style={{height:"100%",background:"rgba(255,255,255,0.8)",width:Math.min(100,(totExp/(md.income||1))*100)+"%",borderRadius:"100px",transition:"width .5s ease"}}/></div>{parseFloat(savPct)>=30&&<div style={{marginTop:"8px",fontSize:"11px",opacity:.9,textAlign:"center",letterSpacing:".5px"}}>You saved {savPct}% this month. Excellent control.</div>}{parseFloat(savPct)>=20&&parseFloat(savPct)<30&&<div style={{marginTop:"8px",fontSize:"11px",opacity:.85,textAlign:"center",letterSpacing:".5px"}}>Saving {savPct}% — on target. Keep going.</div>}</>}
          </div>
