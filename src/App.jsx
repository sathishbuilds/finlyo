...
const INVEST_CATS=[{id:"equity",label:"Equity / Stocks",color:"#3B6FD4"},{id:"mf",label:"Mutual Funds",color:"#6B8FD4"},{id:"sip",label:"SIP",color:"#4AAED4"},{id:"gold",label:"Gold / Silver",color:"#C8961E"},{id:"ppf",label:"PPF",color:"#2E8B57"},{id:"pf",label:"PF / EPF",color:"#52A878"},{id:"largecap",label:"Large Cap",color:"#5577CC"},{id:"midcap",label:"Mid Cap",color:"#7B66B8"},{id:"smallcap",label:"Small Cap",color:"#A05090"},{id:"fd",label:"Fixed Deposit",color:"#4A7FA8"},{id:"realestate",label:"Real Estate",color:"#B87333"},{id:"crypto",label:"Crypto",color:"#C0622A"},{id:"bonds",label:"Bonds",color:"#3A9E8E"},{id:"other",label:"Other",color:"#7A8FA0"}];
...
// Dual display component — exact value + compact hint
const FmtDual=({n,sym,cur,size="15px",color,C})=>{
  if(n===undefined||n===null||isNaN(n))return <span style={{fontSize:size,fontWeight:"700",color:color||"inherit",fontFamily:"'DM Mono',monospace"}}>{sym}0</span>;
  const exact=fmt(n,sym,cur);
  const compact=fmtCompact(n,sym,cur);
  const showBoth=exact!==compact&&Math.abs(n)>=10000;
  return(
    <span style={{display:"inline-flex",flexDirection:"column",lineHeight:1.2}}>
      <span style={{fontSize:size,fontWeight:"700",color:color||"inherit",fontFamily:"'DM Mono',monospace"}}>{exact}</span>
      {showBoth&&<span style={{fontSize:"9px",color:C?C.inkLo:"#8FA0B5",fontFamily:"'DM Mono',monospace",marginTop:"1px",opacity:.75}}>~{compact}</span>}
    </span>
  );
};
...
function InsightCard({insight,C}){
  // positive = green, everything else = soft neutral with subtle left accent
  const isPos=insight.type==="positive";
  const isNeg=insight.type==="negative";
  const isWarn=insight.type==="warning";
  const bg=isPos?C.greenSoft:C.white;
  const border=isPos?C.green+"33":C.border;
  const iconColor=isPos?C.green:isNeg?C.red:isWarn?C.amber:C.inkLo;
  const leftBar=isPos?C.green:isNeg?C.red:isWarn?C.amber:C.borderMid;
  return(
    <div style={{background:bg,border:"1px solid "+border,borderRadius:"11px",padding:"10px 12px",display:"flex",alignItems:"flex-start",gap:"10px",marginBottom:"7px",borderLeft:"3px solid "+leftBar}}>
      <span style={{fontSize:"13px",color:iconColor,fontWeight:"700",flexShrink:0,marginTop:"1px",fontFamily:"'DM Mono',monospace"}}>{insight.icon}</span>
...
function Finlyo({user,darkMode,onOpenSettings}){
  ...
  const trendInsights=genTrendInsights(budgets,mo,yr,C);
      ...
      <main style={{padding:"14px 13px 56px",maxWidth:"640px",margin:"0 auto"}} onClick={()=>{showCur&&setShowCur(false);showMore&&setShowMore(false);}}>
        ...
        {tab==="Budget"&&<div className="fade">
          ...
          <div className="hc" style={{background:darkMode?"linear-gradient(135deg,#1A3A6A,#2A5090)":"linear-gradient(135deg,#2B6CB0,#3A7FCC)",borderRadius:"16px",padding:"18px 16px",marginBottom:"10px",color:"#fff",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{marginBottom:"12px"}}><FmtDual n={bal} sym={S} cur={cur.code} size="28px" color="#fff" C={C}/></div>
        ...
        {tab==="Invest"&&<div className="fade">
          ...
          <div className="hc" style={{background:darkMode?"linear-gradient(135deg,#1A2535,#0D1520)":"linear-gradient(135deg,#0D1520,#1E293B)",borderRadius:"16px",padding:"16px",marginBottom:"9px",color:"#fff",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{marginBottom:"3px"}}><FmtDual n={totCur} sym={S} cur={cur.code} size="26px" color="#fff" C={C}/></div>
          ...
          {pieData.length>0&&(()=>{
            // Custom label renderer — shows % next to each slice
            const renderLabel=({cx,cy,midAngle,innerRadius,outerRadius,percent})=>{
              ...
              return(
                <text x={x} y={y} fill={darkMode?"#94A3B8":"#5A6B80"} textAnchor={x>cx?"start":"end"} dominantBaseline="central" fontSize="9" fontFamily="'DM Mono',monospace" fontWeight="600">
                  {(percent*100).toFixed(0)}%
                </text>
              );
            };
            return(
              <div style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"14px",marginBottom:"9px"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                  <div style={{width:"160px",height:"160px",flexShrink:0}}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={pieData} cx="50%" cy="50%" innerRadius={42} outerRadius={62} dataKey="value" paddingAngle={2} labelLine={false} label={renderLabel}>
                          {pieData.map((e,i)=><Cell key={i} fill={e.color}/>)}
                        </Pie>
                        <Tooltip
                          formatter={(v,name)=>[fmt(v,S,cur.code)+" ("+fmtCompact(v,S,cur.code)+")",name]}
                          contentStyle={{background:C.white,border:"1px solid "+C.border,borderRadius:"8px",fontSize:"11px",fontFamily:"'DM Mono',monospace"}}
                        />
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    {pieData.map(d=>{
                      ...
                      return(
                        <div key={d.id} style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"7px"}}>
                          ...
                          <div style={{textAlign:"right",flexShrink:0}}>
                            <div style={{fontSize:"11px",fontWeight:"700",color:C.ink,fontFamily:"'DM Mono',monospace"}}>{p2}%</div>
                            <div style={{fontSize:"9px",color:C.inkLo,fontFamily:"'DM Mono',monospace"}}>{fmtCompact(d.value,S,cur.code)}</div>
                          </div>
                        </div>
                      );
                    })}
        ...
        {tab==="Goals"&&<div className="fade">
          ...
          :goals.map(g=>{const p=Math.min(100,g.target>0?(g.saved/g.target)*100:0);return<div key={g.id} className="hc" style={{background:C.white,border:"1px solid "+C.border,borderRadius:"14px",padding:"15px",marginBottom:"9px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"10px"}}><div><div style={{fontSize:"10px",color:C.blue,fontFamily:"'DM Mono',monospace",letterSpacing:"1px",marginBottom:"2px"}}>{g.label.toUpperCase()}</div><div style={{fontSize:"15px",fontWeight:"700",color:C.ink}}>{g.name}</div>{g.deadline&&<div style={{fontSize:"10px",color:C.inkLo,fontFamily:"'DM Mono',monospace",marginTop:"1px"}}>Target: {g.deadline}</div>}
                ...
                })()}</div><div style={{textAlign:"right"}}><FmtDual n={g.target} sym={S} cur={cur.code} size="15px" color={C.ink} C={C}/><button onClick={()=>delGoal(g.id)} style={{background:"none",border:"none",color:C.inkLo,cursor:"pointer",fontSize:"11px",marginTop:"3px"}}>Remove</button></div></div>
        ...
        {tab==="Overview"&&<div className="fade">
          ...
          {/* ── STORY CARD ── */}
          {story&&<div style={{background:"linear-gradient(135deg,"+C.ink+",#1E293B)",borderRadius:"16px",padding:"18px 18px",marginBottom:"10px",position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:"-20px",right:"-20px",width:"80px",height:"80px",borderRadius:"50%",background:"rgba(255,255,255,0.03)"}}/>
            <div style={{fontSize:"9px",color:"rgba(255,255,255,0.45)",fontFamily:"'DM Mono',monospace",letterSpacing:"2px",marginBottom:"10px"}}>{MONTHS[mo].toUpperCase()} IN REVIEW</div>
            <div style={{fontSize:"14px",color:"rgba(255,255,255,0.9)",lineHeight:1.8,fontStyle:"italic",fontWeight:"400"}}>{story}</div>
          </div>}
          ...
          {insights.length>0&&(()=>{
            ...
            return<div style={{marginBottom:"10px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"9px"}}>
                ...
                {insights.length>2&&<button onClick={()=>setShowAll(s=>!s)} style={{background:"none",border:"none",color:C.blue,cursor:"pointer",fontFamily:"'DM Mono',monospace",fontSize:"10px",letterSpacing:"1px"}}>{showAll?"LESS ↑":"MORE ↓"}</button>}
              </div>
              {visible.map((ins,i)=><InsightCard key={i} insight={ins} C={C}/>)}
            </div>;
          })()}
