                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={pieData} cx="50%" cy="50%" innerRadius={42} outerRadius={62} dataKey="value" paddingAngle={2}
                      label={({percent})=>percent>=0.06?(percent*100).toFixed(0)+"%":null}
                      labelLine={false} style={{fontSize:"9px",fill:darkMode?"#94A3B8":"#5A6B80",fontFamily:"DM Mono"}}>
                      {pieData.map((e,i)=><Cell key={i} fill={e.color}/>)}
                    </Pie>
                    <Tooltip formatter={(v,name)=>[fmt(v,S,cur.code),name]} contentStyle={{background:C.white,border:"1px solid "+C.border,borderRadius:"8px",fontSize:"11px"}}/>
                  </PieChart>
                </ResponsiveContainer>
              </div>
              <div style={{flex:1,minWidth:0}}>
